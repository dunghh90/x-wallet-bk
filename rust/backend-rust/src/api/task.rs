...
impl ResponseError for TaskError {
  ...
  fn status_code(&self) -> StatusCode {
    match self {
      ...
      TaskError::TaskCreateFailure => StatusCode::FAILED_DEPENDENCY,
...
// pub async fn get_task(task_indentifier: Path<TaskIdentifier>, body: Json<Struct>) -> Json<String> {
// pub async fn get_task(task_indentifier: Path<TaskIdentifier>) -> Result<Json<Task>, TaskError> {
pub async fn get_task(
  task_indentifier: Path<TaskIdentifier>, 
  ddb_repo: Data<DDBRepository>,
) -> Result<Json<Task>, TaskError> {
  // return Json("Json hello world!!!".to_string());
  // return Json(task_indentifier.into_inner().task_global_id);
  let task = ddb_repo.get_task(
    task_indentifier.into_inner().task_global_id
  ).await;
  match task {
...
pub async fn submit_task(
  ...
  let task_indentifier = task.get_global_id();
  match ddb_repo.put_task(task).await {
    Ok(()) => Ok(Json(TaskIdentifier {task_global_id: task_indentifier})),
    Err(_) => Err(TaskError::TaskCreateFailure),
  }
}
