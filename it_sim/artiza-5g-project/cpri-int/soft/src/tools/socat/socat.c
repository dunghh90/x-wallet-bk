...
static volatile sig_atomic_t fj_plane_save_file_clean_flg = 0;
static volatile sig_atomic_t fj_plane_save_count_max = 1024;
static pid_t fj_socat_pid = 0;

static void f_socat_handler(int val){
	fj_plane_save_file_clean_flg = 1;
	fj_plane_save_count_max = 10000;
	return;
}
...
int main(int argc, const char *argv[]) {
   ...
   while (arg1[0] && (arg1[0][0] == '-')) {
	 ...
	 socat_opts.verbose = true; break;
      case 'z':  if (arg1[0][2])  { socat_opt_hint(stderr, arg1[0][1], arg1[0][2]); Exit(1); }
	 socat_opts.mpbose = true; break;
   ...
   signal(SIGRTMIN, f_socat_handler);
...
static int
   ...
   if (righttoleft) {
      sprintf(buff, "\n%s%s length="F_Zu" from=%lu to=%lu pid=%d bt=%lu.%lu\n",
	      prefixrtol, timestamp, bytes, numrtol, numrtol+bytes-1,fj_socat_pid,boottime.tv_sec,boottime.tv_nsec);
      numrtol+=bytes;
   } else {
      sprintf(buff, "\n%s%s length="F_Zu" from=%lu to=%lu pid=%d bt=%lu.%lu\n",
	      prefixltor, timestamp, bytes, numltor, numltor+bytes-1,fj_socat_pid,boottime.tv_sec,boottime.tv_nsec);
      numltor+=bytes;
   }
   fputs(buff, file);
   return 0;
...
#define FJ_RPCS_LOGNAME			"/var/log/netconfd-pro.log.rpcs"

static char fj_rpcs_logname_base[64];
static int plane_sd = -1;
static char eth_frame[1514];
static int plane_maxlen;
static int plane_pos;
static FILE* plane_files = NULL;
static int plane_save_count = 0;
long int fj_plane_save_file_max  = 0x1000000;	/* 16M */
static int plane_save_stupflg = 0;
static unsigned long plane_log_rotate_cnt = 0;
static int plane_modecheck_fd = -1;
static void* plane_modecheck_addr = (void*)-1;

static char plane_mac_addr1[6];
static char plane_mac_addr2[6];
...
unsigned int fj_func_mirror_check(
	unsigned int* ether_type ,
	unsigned int* dst_mac_h, 
	unsigned int* dst_mac_l,
	unsigned int* src_mac_h,
	unsigned int* src_mac_l
)
{
	unsigned int mode_val;

	if(plane_modecheck_fd == -1){
		if( ( plane_modecheck_fd = open("/dev/mem",O_RDWR) ) == -1){
			return 0;
		}
		if((plane_modecheck_addr = mmap(NULL,sysconf(_SC_PAGESIZE),PROT_READ|PROT_WRITE,MAP_SHARED,plane_modecheck_fd,0xA0000000)) == (void*)-1 ){	/* pgr0203 */
			close(plane_modecheck_fd);
			return 0;
		}
	}
	mode_val = (unsigned int)*((unsigned *)(plane_modecheck_addr + 0xC20));
	if((mode_val & 0x01) != 0){
		*ether_type = (unsigned int)*((unsigned *)(plane_modecheck_addr + 0xC24));
		*dst_mac_h  = (unsigned int)*((unsigned *)(plane_modecheck_addr + 0xC30));
		*dst_mac_l  = (unsigned int)*((unsigned *)(plane_modecheck_addr + 0xC34));
		*src_mac_h  = (unsigned int)*((unsigned *)(plane_modecheck_addr + 0xC38));
		*src_mac_l  = (unsigned int)*((unsigned *)(plane_modecheck_addr + 0xC3C));
	}
	return (mode_val & 0x1);
}

void fj_func_mirror_send( int len , unsigned char* data ,bool righttoleft)
{
	struct ifreq			ifrq;
	...
	if(( check = fj_func_mirror_check(&ether_type,&dst_mac_h,&dst_mac_l,&src_mac_h,&src_mac_l)) == 1){

		header = (struct ether_header*)&eth_frame[0];
		...
		if(plane_sd < 0){
			...
			header->ether_type     = htons((unsigned short)ether_type);
			plane_maxlen = sizeof(eth_frame) - sizeof(*header);
			plane_pos = sizeof(*header);

			plane_mac_addr1[0] = (dst_mac_h & 0xFF00) >> 8;
			plane_mac_addr1[1] = (dst_mac_h & 0xFF);
			plane_mac_addr1[2] = (dst_mac_l & 0xFF000000) >> 24;
			plane_mac_addr1[3] = (dst_mac_l & 0xFF0000)   >> 16;
			plane_mac_addr1[4] = (dst_mac_l & 0xFF00)     >> 8;
			plane_mac_addr1[5] = (dst_mac_l & 0xFF);

			plane_mac_addr2[0] = (src_mac_h & 0xFF00) >> 8;
			plane_mac_addr2[1] = (src_mac_h & 0xFF);
			plane_mac_addr2[2] = (src_mac_l & 0xFF000000) >> 24;
			plane_mac_addr2[3] = (src_mac_l & 0xFF0000)   >> 16;
			plane_mac_addr2[4] = (src_mac_l & 0xFF00)     >> 8;
			plane_mac_addr2[5] = (src_mac_l & 0xFF);

		}

		if(righttoleft){
			memcpy(header->ether_dhost,plane_mac_addr1,6);
			memcpy(header->ether_shost,plane_mac_addr2,6);
		}else{
			memcpy(header->ether_dhost,plane_mac_addr2,6);
			memcpy(header->ether_shost,plane_mac_addr1,6);
		}

		if(totallen > plane_maxlen){
			dlen = plane_maxlen;
		}else{
			dlen = totallen;
		}
		...
		while(1){
			memcpy(&eth_frame[plane_pos],tmp,dlen);
			slen = dlen + sizeof(*header);
			if(( ret = send(plane_sd, eth_frame, (size_t)slen, 0)) < 0){
				perror("send");
				close(plane_sd);
				plane_sd = -1;
				break;
	...
	if(plane_files == NULL){
		long int filelen;
		memset(fj_rpcs_logname_base,0,sizeof(fj_rpcs_logname_base));

		struct timespec boottime;
		(void)clock_gettime( CLOCK_MONOTONIC_RAW   , &boottime);

		if(plane_save_stupflg == 0){
			snprintf(fj_rpcs_logname_base,sizeof(fj_rpcs_logname_base),"%s.stup",FJ_RPCS_LOGNAME);
			plane_files = fopen(fj_rpcs_logname_base,"a+");
			if(plane_files == NULL){
				return;
			}
			fj_socat_pid = getpid();
			fprintf(plane_files,"\n");	/* 書き込み位置を最後まで持っていく */
			filelen = ftell(plane_files);
			if(filelen > fj_plane_save_file_max ){	/* limit over */
				/* limitを超えたら、新規で作成し直す */
				fclose(plane_files);
				plane_files = fopen(fj_rpcs_logname_base,"w");
				if(plane_files == NULL){
					return;
				}
				fprintf(plane_files,"# *** log reopen(%ld) at:%lu.%lu by pid:%d *** \n",filelen,boottime.tv_sec,boottime.tv_nsec,fj_socat_pid);
			}else{
				fprintf(plane_files,"# *** log open(%ld) at:%lu.%lu by pid:%d *** \n",filelen,boottime.tv_sec,boottime.tv_nsec,fj_socat_pid);
			}
		}else{
			plane_log_rotate_cnt++;
			snprintf(fj_rpcs_logname_base,sizeof(fj_rpcs_logname_base),"%s.log.%d",FJ_RPCS_LOGNAME,fj_socat_pid);
			plane_files = fopen(fj_rpcs_logname_base,"w");
			if(plane_files == NULL){
				return;
			}
			fprintf(plane_files,"# *** log rotate(%lu) at:%lu.%lu by pid:%d *** \n",plane_log_rotate_cnt,boottime.tv_sec,boottime.tv_nsec,fj_socat_pid);
		}
	}

	size_t i = 0;
	xioprintblockheader(plane_files, len, righttoleft);
	while (i < (size_t)len) {
		int c = data[i];
		switch (c) {
		case '\a' : fputs("\\a", plane_files); break;
		case '\b' : fputs("\\b", plane_files); break;
		case '\t' : fputs("\t", plane_files); break;
		case '\n' : fputs("\n", plane_files); break;
		case '\v' : fputs("\\v", plane_files); break;
		case '\f' : fputs("\\f", plane_files); break;
		case '\r' : fputs("\\r", plane_files); break;
		case '\\' : fputs("\\\\", plane_files); break;
		default:
		   if (!isprint(c))
			c = '.';
	...
	fflush(plane_files);
	plane_save_count++;
	if((plane_save_count >= (int)fj_plane_save_count_max) || (fj_plane_save_file_clean_flg != 0)){
		fclose(plane_files);
		if(plane_save_stupflg == 0){
			plane_save_stupflg = 1;
		} else {
			char rpcs_logname_base[75];
			memset(rpcs_logname_base,0,sizeof(rpcs_logname_base));
			snprintf(rpcs_logname_base,sizeof(rpcs_logname_base),"%s.old.%d",FJ_RPCS_LOGNAME,fj_socat_pid);
			if(plane_save_stupflg > 1 ){
				remove(rpcs_logname_base);
			}
			rename(fj_rpcs_logname_base,rpcs_logname_base);
			plane_save_stupflg = 2;
		}
		plane_save_count = 0;
		fj_plane_save_file_clean_flg = 0;
		plane_files = NULL;
	}
	return;
}
	 ...
	 if (bytes > 0) {
	    ...
	    if (socat_opts.verbose && socat_opts.verbhex) {
		  ...
		  fprintf(stderr, " %02x", (*buff)[i]);
	       ...
	       fputc('\n', stderr);
	    } else if (socat_opts.mpbose) {
			fj_func_mirror_send(bytes,*buff,righttoleft);
		}
