 ...
 *  @skip	$Id:$
 *  @file	rrhlib.c
 *  @brief	rrh command library routine
...
{
	rrhlib_dmsg("rrh lib clean \n");
	if ( rrhlib_assignflg > 0 )
	{
		(void)BPF_RM_SVRM_UNASSIGN(D_DBG_TP_PROCESS_DESC);
	}
	if( rrhlib_assignflg != rrhlib_unassignflg ){
		rrhlib_dmsg("rrh lib assign , unassign error \n",rrhlib_assignflg,rrhlib_unassignflg);
	}
	rrhlib_assignflg = 0;
	rrhlib_unassignflg = 0;
	rrhlib_dmsg("rrh lib clean OK \n");
	return;
}
...
mrb_value rrhlib_sigqueue(mrb_state* mrb, mrb_value self)
{
	...
	mrb_get_args(mrb, "Si|i", &str, &pid , &value );
	...
	{
		rrhlib_dmsg("Sigqueue unknown:%s pid:%d value:%x \n",RSTRING_PTR(str),pid,value);
		return mrb_false_value();
	}
	send_value.sival_int = value;
	int rtn = sigqueue( pid, signo ,send_value );
	rrhlib_dmsg("Sigqueue rtn:%d,%d,name:%s pid:%d value:%x \n",rtn,errno,RSTRING_PTR(str),pid,value);
...
mrb_value rrhlib_assign(mrb_state* mrb, mrb_value self)
{
	...
	rtn_bpf = BPF_RM_SVRM_ASSIGN(D_DBG_TP_PROCESS_DESC);
 ...
 * @date        2013/12/17
 * @warning     本関数はスクリプトからのコール関数である.
 * @warning     スクリプト上でのモジュール呼び出し名 : Rrh.psalive
 * @FeatureID   N/A
 * @Bug_No      N/A
 * @CR_No       N/A
 * @TBD_No      N/A
...
mrb_value rrhlib_psalive(mrb_state* mrb, mrb_value self)
{
	mrb_value	name;
	CHAR					str[10];
	CHAR					command[128];
	...
	snprintf(command, 128, "ps -ef | grep -v grep |grep \"%s\" |grep -v defunct |wc -l ", RSTRING_PTR(name));
	if((fp = popen(command, "r")) == NULL){
		rrhlib_dmsg("popen error %s\n",RSTRING_PTR(name));
		return mrb_false_value();
	}
	ret = fgets(str, 10, fp);
	if(ret != NULL){
...
void rrhlib_install(mrb_state* mrb )
{
	...
	rrhlib_dmsg("rrh lib install start \n");
	...
	mrb_define_module_function(mrb, module_class, "psalive" ,   rrhlib_psalive    , MRB_ARGS_REQ(1));
	mrb_define_module_function(mrb, module_class, "exit"  ,    rrhlib_exit        , MRB_ARGS_REQ(1));


	rrhlib_soc_install(mrb,module_class);
	rrhlib_file_install(mrb,module_class);
	rrhlib_device_access_install(mrb,module_class);
	rrhlib_api_install(mrb,module_class);
	rrhlib_tbl_access_install(mrb,module_class);

	rrhlib_dmsg("rrh lib install OK \n");
	return;
}
