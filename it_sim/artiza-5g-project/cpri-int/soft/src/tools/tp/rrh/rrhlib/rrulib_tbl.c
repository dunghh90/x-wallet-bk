...
static T_RRH_DU_CMD_INFO* dulib_cmdshm_addr = NULL;
...
unsigned int rrhlib_init_cmdshm(mrb_state* mrb){
	...
	if( (endcd = BPF_RU_IPCM_PROCSHM_ADDRGET(E_RRH_SHMID_APL_TP_CMD_INFO,(VOID **)&dulib_cmdshm_addr,&errcd)) != BPF_RU_IPCM_OK ){
		rrhlib_dmsg("Memory Get key NG = %d ,errcd = %d \n",endcd,errcd);
		return D_RRH_NG;
	}

	endcd = BPF_RU_IPCM_PROCSEM_TAKE(E_RRH_SEMID_APL_TP_CMD_INFO,BPF_RU_IPCM_LOCK_RW, BPF_RU_IPCM_WAIT, &dtime, &errcd);
	if(BPF_RU_IPCM_OK != endcd){
		rrhlib_dmsg("sem take NG = %d ,errcd = %d \n",endcd,errcd);
		return D_RRH_NG;
	}

	if (dulib_cmdshm_addr->com_info.pid == 0) {
	...
	dulib_cmdshm_addr->com_info.pid   = pid;
	...
	endcd =  BPF_RU_IPCM_PROCSEM_GIVE(E_RRH_SEMID_APL_TP_CMD_INFO,BPF_RU_IPCM_LOCK_RW, &errcd);
	if(BPF_RU_IPCM_OK != endcd){
...
extern unsigned int rrhlib_assignflg;
...
{
	(VOID)BPF_RM_SVRM_ASSIGN(D_DBG_TP_PROCESS_DESC);
	rrhlib_assignflg++;	
