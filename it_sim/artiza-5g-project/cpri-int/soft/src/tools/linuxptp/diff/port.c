...
static int port_tx_sync(struct port *p, struct address *dst)
{
	...
	if (err) {
		pr_err("port %hu: send sync failed %d", portnum(p),err);
		goto out;
	}
	if (p->timestamping == TS_ONESTEP || p->timestamping == TS_P2P1STEP) {
		goto out;
	...
	fup->follow_up.preciseOriginTimestamp = tmv_to_Timestamp(timespec_to_tmv(wts));
...
static int process_delay_req(struct port *p, struct ptp_message *m)
{
	...
	struct timespec wts;
	clock_gettime(CLOCK_REALTIME, &wts);
	msg->delay_resp.receiveTimestamp = tmv_to_Timestamp(timespec_to_tmv(wts));
