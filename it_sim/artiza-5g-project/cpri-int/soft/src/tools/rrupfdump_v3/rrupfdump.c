...
unsigned int func_seqdump(FILE* fp,int len,char* datap,int orglen,int mode , int dspmode)
{
	...
	unsigned int				kind;
	int							userlen;
	pcap_rrupf_ipheader* 		ip_p;
	pcap_rrupf_userheader* 		user_p;
	pcap_rrupf_userheader_if* 	user_p_if;
	int rtn = D_RRUPKTDUMP_TYPE_UNKNOWN;

	if(len < (sizeof(pcap_rrupf_ipheader) + sizeof(pcap_rrupf_ethheader)))
	{
		fprintf(fp,"ERROR len is %d  \n",len);
		return (rtn);
	}
	ip_p = (pcap_rrupf_ipheader*)(datap + sizeof(pcap_rrupf_ethheader));
	if( ntohl((unsigned int)ip_p->h_ip.ip_dst.s_addr) > D_RRUPLOG_IP_OFS)
	{

		index = func_iphead_dsp(fp,(pcap_data_timeoffs_t* )NULL,ip_p,&subindex,&kind);
		userlen = len - ( sizeof(pcap_rrupf_ethheader) + sizeof(pcap_rrupf_ipheader));
		if(userlen > 0)
		{
			user_p = (pcap_rrupf_userheader*)(((char*)ip_p) + sizeof(*ip_p));
			if((ntohl((unsigned int)ip_p->h_ip.ip_dst.s_addr)) >= D_RRUPLOG_IP_OFS_MP){
				subindex = (ntohl((unsigned int)ip_p->h_ip.ip_dst.s_addr)) & 0x7;
				rtn = func_userdata_dsp_mp(fp,user_p,userlen,index,subindex,ntohl((unsigned int)ip_p->h_ip.ip_src.s_addr),kind);
			}else{
				rtn = func_userdata_dsp(fp,user_p,userlen,index,subindex,ntohl((unsigned int)ip_p->h_ip.ip_src.s_addr),kind);
			}
		}
		fprintf(fp,"\n");
	}
	else
	{
		...
		procindex = func_iphead_dsp_if(fp,(pcap_data_timeoffs_t* )NULL,ip_p);
		userlen = len - ( sizeof(pcap_rrupf_ethheader) + sizeof(pcap_rrupf_ipheader));
		if(userlen < sizeof(pcap_rrupf_userheader_if))
		{
			fprintf(fp,"ERROR userlen is %d  \n",userlen);
			return (rtn);
		}
		user_p_if = (pcap_rrupf_userheader_if*)(((char*)ip_p) + sizeof(*ip_p));
		rtn = func_userdata_dsp_if(fp,user_p_if,userlen,procindex);
		fprintf(fp,"\n");
		if(mode >= 1)
		{
			/* mid */
			if(mode == 1)
			{
				maxlen = D_RRUPKTDSP_MSIZE;	/* max dsp size */
				/* only user data */
				udatap = ((char*)user_p_if) + sizeof(pcap_rrupf_userheader_if);
				dsplen = userlen - sizeof(pcap_rrupf_userheader_if) ;
			}
			func_datadsp(
				fp,
				dsplen,
...
{ 
	...
	/* レングス異常 */
	len = rph->caplen;
	if(len < (int)(sizeof(pcap_rrupf_ethheader) + sizeof(pcap_rrupf_ipheader)))
	{
		func_gmsg("length mismatch \n");
		return 0;
	}
	len -= (sizeof(pcap_rrupf_ethheader) + sizeof(pcap_rrupf_ipheader));
	eth_p = (pcap_rrupf_ethheader *)rpmsg;
	ip_p = (pcap_rrupf_ipheader *)(((char* )eth_p) + sizeof(*eth_p));
	/* RAW socket以外は無視 */
	if(ip_p->h_ip.ip_p != IPPROTO_RAW )
	{
		func_gmsg("raw packet mismatch <ip_p is %d> \n",ip_p->h_ip.ip_p);
		return 0;
	}
	unsigned int dip = ntohl((unsigned int)ip_p->h_ip.ip_dst.s_addr);
	if(( dip > D_RRUPLOG_IP_MAX ) || ( dip < D_RRUPLOG_IP_MIN  ))
	{
		func_gmsg("unknown ip %x \n",dip);
		return 0;
	}
	...
	{	
		if(len > 0)
		{
			if( ntohl((unsigned int)(ip_p->h_ip.ip_dst.s_addr)) > D_RRUPLOG_IP_OFS )
			{
				...
				index  = func_iphead_dsp(stdout,(pcap_data_timeoffs_t*)&(rph->ts),ip_p,&subindex,&kind);
				user_p = (pcap_rrupf_userheader *)(((char* )ip_p) + sizeof(*ip_p));

				if( ntohl((unsigned int)(ip_p->h_ip.ip_dst.s_addr)) >= D_RRUPLOG_IP_OFS_MP){
					subindex = (ntohl((unsigned int)(ip_p->h_ip.ip_dst.s_addr))) & 0x7;
					(void)func_userdata_dsp_mp(stdout,user_p,len,index,subindex,ntohl((unsigned int)(ip_p->h_ip.ip_src.s_addr)),kind);
				}
				else{
					(void)func_userdata_dsp(stdout,user_p,len,index,subindex,ntohl((unsigned int)(ip_p->h_ip.ip_src.s_addr)),kind);
				}
				fprintf(stdout,"\n");
			}
			else
			{
				...
				char*						udatap;
				int							dsplen;
				procindex = func_iphead_dsp_if(stdout,(pcap_data_timeoffs_t*)&(rph->ts),ip_p);
				user_p_if = (pcap_rrupf_userheader_if *)(((char* )ip_p) + sizeof(*ip_p));
				(void)func_userdata_dsp_if(stdout,user_p_if,len,procindex);
				fprintf(stdout,"\n");
				udatap = ((char*)user_p_if) + sizeof(pcap_rrupf_userheader_if);
				dsplen = len - sizeof(pcap_rrupf_userheader_if) ;
				func_datadsp(
					stdout,
					dsplen,
					udatap,
	...
	for(count = 0 ; ; count++)
	{
		for(fcount = 0 ; fcount < pfmaxcount ; fcount++)
		{
			...
			while((rtn = pcap_next_ex(rruplog_pcap_p,&rph,(const unsigned char **)&butp)) >= 0)
			{
				...
				rtcount += rtn;
				if((gcount = func_rrupacketcap_callback((unsigned char*)drlp,rph,butp)) < 0)
				{
					/* ng */
...
int main (int argc , char* argv[])
{
	...
	while((rtn = getopt(argc,argv,"hdgvtn:c:e:f:F:N:C:s:l:m:p:b:")) != -1)
	{
		switch(rtn)
		{
	...
	while(filelen > 0)
	{
		...
		local_sec = (time_t)fdatahead.ts.tv_sec;
		localtime_r((const time_t *)&local_sec,&lctimep);
		fprintf(seqfp,"%04d/%02d/%02d %02d:%02d:%02d.%06d(%lu.%06ld):",
