 ...
 *						-# テーブルの送信先プロセスQIDに引数の送信先プロセスQIDを設定.
 *						-# アサートログを出力.
 *						-# 正常終了.
 *					-# 追加するEventID＆送信先プロセスQIDがテーブルに登録済みか判定.
 *						- 既に登録済みの場合.
 *						-# アサートログを出力.
 *						-# 異常終了.
 *					-# 異常終了.
...
UINT f_send_addMsgMap(UINT uiEventID, UINT uiSignalKind, UINT uiQueID)
{
	...
	uiRet = f_cmu_com_addSearch(uiEventID, uiSignalKind, uiQueID, &f_sendw_route);
