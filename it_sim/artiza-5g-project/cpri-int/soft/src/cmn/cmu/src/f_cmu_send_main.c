 ...
 *					-# [初期化状態]へ状態遷移.
 *					-# 初期化処理関数コール(f_send_init()).
 *					-# MSG待ち無限Loop処理へ移行関数コール(BPF_RU_IPCM_MSGQ_RECV()).
 *					-# MSG受信時は解析処理を実施関数コール(f_send_anlz()).
...
VOID f_send_main(T_CMU_CREATE_THREAD *tRcvMsg)
{
	...
	/* 初期状態を設定	*/
	f_sendw_thdState = D_CMU_THD_STS_INIT;
	
	/* 初期化処理		*/
	(VOID)f_cmu_com_init(tRcvMsg, &f_sendw_route, &f_sendw_ProcTbl, &f_sendw_thdState);
	...
	{
		/* スレッド間MSG受信待ち	*/
		ret = BPF_RU_IPCM_MSGQ_RECV(	tRcvMsg->uiThreadQID,		/* スレッドQID					*/
										0,							/* 応答待ち時間					*/
		...
		if(BPF_RU_IPCM_OK == ret){
			/* 受信MSG解析処理			*/
			(VOID)f_send_anlz((VOID*)rcvMsg_p);
		}
	}
}
