 ...
 *					- 上記以外場合.
 *						-# 処理継続.
 *					-# 削除するEventID＆送信先スレッドQIDがテーブルに登録済みか判定.
 *					- 登録ありの場合.
 *						-# EventIDに0を設定.
 *						-# 送信先スレッドQIDに0を設定.
 *						-# アサートログを出力.
 *						-# 削除済みフラグをONに設定.
 *						-# メッセージ送信先情報管理テーブルの登録数を-1する.
 *					-# Mutex Unlock.
 *					- Mutex Unlock失敗の場合.
 *						-# アサートログを出力し、異常終了.
 *					-# 正常終了.
...
UINT f_recv_delMsgMap(UINT uiEventID, UINT uiSignalKind, UINT uiQueID)
{
	...
	uiRet = f_cmu_com_delSearch(uiEventID, uiSignalKind, uiQueID, &f_recvw_route);
