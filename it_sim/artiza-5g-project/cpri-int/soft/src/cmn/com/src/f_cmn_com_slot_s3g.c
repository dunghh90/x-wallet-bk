/*!
 * @skip  $ld:$
 * @file  f_cmn_com_slot_s3g.c
 * @brief 共有メモリ取得/設定(スロット情報(S3G))
 ...
 * @date    2016/10/11 KCN)hfuku 16B機能追加
 */
INT f_cmn_com_slot_s3g_get(USHORT linkno, T_RRH_SLOT_S3G_RE *pOutData)
{
    UINT offset  = (linkno - 1) * sizeof(*pOutData);    /* 共有メモリオフセット */
    UINT shm_num = (UINT)E_RRH_SHMID_APL_SLOT_S3G;      /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_SLOT_S3G;      /* セマフォナンバー     */
 ...
 * @date    2016/10/11 KCN)hfuku 16B機能追加
 */
INT f_cmn_com_slot_s3g_set(USHORT linkno, T_RRH_SLOT_S3G_RE *pInData)
{
