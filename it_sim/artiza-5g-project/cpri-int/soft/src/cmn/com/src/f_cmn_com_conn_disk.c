 ...
 */
INT f_cmn_com_conn_disk_get(T_RRH_CONN_DISC *pOutData)
{
    UINT shm_num = (UINT)E_RRH_SHMID_APL_T_RRH_CONN_DISC;   /* 共有メモリナンバー */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_T_RRH_CONN_DISC;   /* セマフォナンバー   */
 ...
 * @date    2015/08/17 TDI)satou 共通の関数をコールするように見直し
 */
INT f_cmn_com_conn_disk_set(T_RRH_CONN_DISC *pInData)
{
    UINT shm_num = (UINT)E_RRH_SHMID_APL_T_RRH_CONN_DISC;   /* 共有メモリナンバー */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_T_RRH_CONN_DISC;   /* セマフォナンバー   */
