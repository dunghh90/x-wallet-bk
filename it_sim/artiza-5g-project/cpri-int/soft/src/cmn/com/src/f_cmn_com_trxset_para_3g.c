/*!
 * @skip  $ld:$
 * @file  f_cmn_com_trxset_para_3g.c
 * @brief 共有メモリ取得/設定(TRX設定パラメータ(3G))
 ...
 */
INT f_cmn_com_trxset_para_3g_get(USHORT carrier, T_TRXSET_PARA_3G *pOutData)
{
    UINT offset  = (carrier - 1) * sizeof(*pOutData);       /* 共有メモリオフセット */
    UINT shm_num = (UINT)E_RRH_SHMID_APL_TRXSET_PARA_3G;    /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_TRXSET_PARA_3G;    /* セマフォナンバー     */

    return f_cmn_com_shm_get_offset(shm_num, sem_num, sizeof(*pOutData), offset, pOutData);
}

/*!
 * @brief   TRX設定パラメータ(3G)設定
 ...
 * @date    2015/08/17 TDI)satou 共通の関数をコールするように見直し
 */
INT f_cmn_com_trxset_para_3g_set(USHORT carrier, T_TRXSET_PARA_3G *pInData)
{
