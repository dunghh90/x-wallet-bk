/*!
 * @skip  $ld:$
 * @file  f_cmn_com_fw_info_fhm.c
 * @brief 共有メモリ取得/設定(FHM保持FWファイル情報(0,1面)
 ...
 */
INT f_cmn_com_fw_info_fhm_get(USHORT side, T_RRH_FW_INFO *pOutData)
{
    UINT offset  = side * sizeof(*pOutData);            /* 共有メモリオフセット */
    UINT shm_num = (UINT)E_RRH_SHMID_APL_FWINFO_FHM;    /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_FWINFO_FHM;    /* セマフォナンバー     */

    return f_cmn_com_shm_get_offset(shm_num, sem_num, sizeof(*pOutData), offset, pOutData);
}

/*!
 * @brief   FHM保持FWファイル情報(0,1面)設定
 ...
 * @date    2015/08/17 TDI)satou 共通の関数をコールするように見直し
 */
INT f_cmn_com_fw_info_fhm_set(USHORT side, T_RRH_FW_INFO *pInData)
{
