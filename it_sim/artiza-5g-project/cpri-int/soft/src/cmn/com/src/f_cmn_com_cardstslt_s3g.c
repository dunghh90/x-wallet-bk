/*!
 * @skip  $ld:$
 * @file  f_cmn_com_cardstslt_s3g.c
 * @brief 共有メモリ取得/設定(REカード状態(S3G)(Slot単位))
 * @date    2016/10/04 KCN)hfuku create
 *
...
/*!
 * @brief   REカード状態(S3G)取得(Slot単位)
 * @note    -
 * @param   linkno      [in]    CPRIリンク番号
 * @param   slt_index   [in]    SLOT番号(Index値:0-15)
 * @param   pOutData    [out]   取得先
 * @return  処理結果
 * @retval  D_RRH_OK
 * @retval  D_RRH_NG
 * @date    2016/10/04 KCN)hfuku create 16B機能追加
 */
INT f_cmn_com_cardstslt_s3g_get(USHORT linkno, USHORT slt_index, T_RRH_CARDST_S3G *pOutData)
{
    UINT shm_num = (UINT)E_RRH_SHMID_APL_CARDST_SLOT_S3G;    /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_CARDST_SLOT_S3G;    /* セマフォナンバー     */
	UINT lnkOfs;
    UINT size;
    INT  ret;
    INT errcd;      /* BPFエラーコード */
    ...
    /*	該当リンクの先頭に移動	*/
    lnkOfs = linkno - 1;
    buf_p += ((lnkOfs * CMD_MAX_CARDNUM) + slt_index); /* 16B：16Slotを加味したオフセット計算に変更 */
 ...
 * @date    2016/10/04 KCN)hfuku create 16B機能追加
 */
INT f_cmn_com_cardstslt_s3g_set(USHORT linkno, USHORT slt_index, T_RRH_CARDST_S3G *pInData)
{
    UINT shm_num = (UINT)E_RRH_SHMID_APL_CARDST_SLOT_S3G;    /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_CARDST_SLOT_S3G;    /* セマフォナンバー     */
	UINT lnkOfs;
