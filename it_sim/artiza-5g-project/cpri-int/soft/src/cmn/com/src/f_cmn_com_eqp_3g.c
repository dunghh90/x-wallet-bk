/*!
 * @skip  $ld:$
 * @file  f_cmn_com_eqp_3g.c
 * @brief 共有メモリ取得/設定(装置構成情報(3G))
 * @date  2015/08/14  uchida Create
 */
 ...
 */
INT f_cmn_com_eqp_3g_get(USHORT linkno, T_RRH_EQP_3G *pOutData)
{
    UINT offset  = (linkno - 1) * sizeof(*pOutData);    /* 共有メモリオフセット */
    UINT shm_num = (UINT)E_RRH_SHMID_APL_EQP_3G;        /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_EQP_3G;        /* セマフォナンバー     */
 ...
 * @date    2015/08/17 TDI)satou 共通の関数をコールするように見直し
 */
INT f_cmn_com_eqp_3g_set(USHORT linkno, T_RRH_EQP_3G *pInData)
{
