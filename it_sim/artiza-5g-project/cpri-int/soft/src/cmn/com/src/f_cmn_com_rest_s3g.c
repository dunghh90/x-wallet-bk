/*!
 * @skip  $ld:$
 * @file  f_cmn_com_rest_s3g.c
 * @brief 共有メモリ取得/設定(RE状態(S3G))
 ...
 */
INT f_cmn_com_rest_s3g_get(USHORT linkno, T_RRH_REST_S3G *pOutData)
{
    UINT offset  = (linkno - 1) * sizeof(*pOutData);    /* 共有メモリオフセット */
    UINT shm_num = (UINT)E_RRH_SHMID_APL_REST_S3G;      /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_REST_S3G;      /* セマフォナンバー     */
 ...
 * @date    2015/08/17 TDI)satou 共通の関数をコールするように見直し
 */
INT f_cmn_com_rest_s3g_set(USHORT linkno, T_RRH_REST_S3G *pInData)
{
