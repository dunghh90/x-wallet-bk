/*!
 * @skip  $ld:$
 * @file  f_cmn_com_re_delay_inf.c
 * @brief 共有メモリ取得/設定(RE単位遅延補正情報)
 ...
 */
INT f_cmn_com_re_delay_inf_get(USHORT linkno, T_RRH_RE_DELAY_INF *pOutData)
{
    UINT offset  = (linkno - 1) * sizeof(*pOutData);    /* 共有メモリオフセット */
    UINT shm_num = (UINT)E_RRH_SHMID_APL_RE_DELAY_INF;  /* 共有メモリナンバー   */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_RE_DELAY_INF;  /* セマフォナンバー     */
 ...
 * @date    2015/08/17 TDI)satou 共通の関数をコールするように見直し
 */
INT f_cmn_com_re_delay_inf_set(USHORT linkno, T_RRH_RE_DELAY_INF *pInData)
{
