/*!
 * @skip  $ld:$
 * @file  f_cmn_com_booteqp_3g.c
 * @brief 共有メモリ取得/設定(CPRIリンク装置構成情報(3G))
 ...
 */
INT f_cmn_com_booteqp_3g_get(T_RRH_EQP_3G *pOutData)
{
	UINT shm_num = (UINT)E_RRH_SHMID_APL_EQP_3G_BOOT;    /* 共有メモリナンバー */
	UINT sem_num = (UINT)E_RRH_SEMID_APL_EQP_3G_BOOT;    /* セマフォナンバー   */

	return f_cmn_com_shm_get(shm_num, sem_num, sizeof(*pOutData), pOutData);
}

/*!
 * @brief   CPRIリンク装置構成情報(3G)設定
 ...
 * @date    2015/10/11 TDIPS)maruyama 共有メモリナンバー,セマフォナンバー変更
 */
INT f_cmn_com_booteqp_3g_set(T_RRH_EQP_3G *pInData)
{
    UINT shm_num = (UINT)E_RRH_SHMID_APL_EQP_3G_BOOT;    /* 共有メモリナンバー */
    UINT sem_num = (UINT)E_RRH_SEMID_APL_EQP_3G_BOOT;    /* セマフォナンバー   */
