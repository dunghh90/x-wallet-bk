...
VOID f_eom_main(void)
{
	...
	/* スレッド名表示	*/
	prctl(PR_SET_NAME, "pf_eom", 0, 0, 0);

	/* BPFへスレッド情報登録	*/
	BPF_RM_SVRM_THREAD_INFO_SET( D_RRH_PROCID_PF,						/* ProcessID			*/
								 D_SYS_THDID_PF_EOM,					/* ThreadID				*//*★*/
								 D_SYS_THDQID_PF_EOM,					/* ThreadQueueID		*//*★*/
								 (unsigned long int)pthread_self());	/* pThreadID			*/

	/* 初期化処理関数Call	*/
	(VOID)f_eom_InitReq( NULL );
	...
	{
		/* メッセージ受信	*/
		ret = BPF_RU_IPCM_MSGQ_RECV(	D_SYS_THDQID_PF_EOM,			/* Thread Qid			*/
										0,								/* 応答待ち時間			*/
										(void **)&msgP,					/* 受信msgポインタ		*/
										&rcvmsgsz );					/* 受信msgサイズ		*/
