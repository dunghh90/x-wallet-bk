...
VOID m_rc_SystermOnOffCont( CMT_TSKIF_CPRIRCV_EXTDATSNDREQ* parm_p )
{
	...
	/* 全配下REの下りIQデータの送信／停止制御　実施 */
	/* TBD */

	/* 局ON/OFF情報状態誌錡 */
	ldwRtn = BPF_HM_DEVC_EEPROM_READ(D_RRH_EEP_BASE_ONOFF, &lcFhmOnOffSta);
	#ifdef FHM_RCT_DEBUG_FOR_IT1
	printf( "[%d]BPF_HM_DEVC_EEPROM_READ : %d \n",__LINE__,ldwRtn );
	#endif
	if( BPF_HM_DEVC_COMPLETE != ldwRtn )
	{
		BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_WARNING, "[Systerm On/Off control sts] EEPROM Read Access Fail.");
		#ifdef FHM_RCT_DEBUG_FOR_IT1
		printf( "[%d]%s end \n",__LINE__,__FUNCTION__);
		#endif
		return;
	...
	/* ON/OFF制御状態更新 */
	lcFhmOnOffSta = (CMD_ON == lcFhmOnOffSta)?CMD_OFF:CMD_ON;
	#ifdef FHM_RCT_DEBUG_FOR_IT1
	printf( "[%d]lcFhmOnOffSta : %d \n",__LINE__,lcFhmOnOffSta );
	#endif

	/* FHMのON/OFF制御状態更新（EEPROM） */
	ldwRtn = BPF_HM_DEVC_EEPROM_WRITE(D_RRH_EEP_BASE_ONOFF, &lcFhmOnOffSta);
	#ifdef FHM_RCT_DEBUG_FOR_IT1
	printf( "[%d]BPF_HM_DEVC_EEPROM_WRITE : %d \n",__LINE__,ldwRtn );
	#endif
	if( BPF_HM_DEVC_COMPLETE != ldwRtn )
	{
		BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_WARNING, "[Systerm On/Off control sts] EEPROM Write Access Fail.");
		#ifdef FHM_RCT_DEBUG_FOR_IT1
		printf( "[%d]%s end \n",__LINE__,__FUNCTION__);
		#endif
		return;
	 ...
	 */
	ltTxOnOff.onoff_fhm = (lcFhmOnOffSta == 1) ? CMD_OFF : CMD_ON;
	...
	/* Build 外部装置データ受信要求送信 */
	m_rc_ExtDataRcvBuildData(parm_p, &response_p, ONOFF_RES_SIG, lwSigsys_rslt);
	
	/* 外部装置データ受信要求送信(状態報告信号) */
	m_rc_ExtDataRcvReqExe(&response_p, RCD_EXTRCVREQ_SYSONOFF, lwSigsys_rslt);
