/*!
 * @skip   $Id$
 * @file   m_lg_Send_GetTroublelog.c
 * @brief  the processing after receiving RE trouble log get request(the state is log sending).
...
/*!
 *  @brief  the function processing after receiving RE trouble log get request(the state is log sending).
 ...
 */
VOID m_lg_Send_GetTroublelog( UCHAR *a_rcv_adr, USHORT a_rec_typ )
{
	cm_Assert(D_RRH_LOG_AST_LV_ENTER, 0, "[m_lg_Send_GetTroublelog] enter");
	...
	cm_Assert(D_RRH_LOG_AST_LV_RETURN, 0,  "[m_lg_Send_GetTroublelog] return");
}
