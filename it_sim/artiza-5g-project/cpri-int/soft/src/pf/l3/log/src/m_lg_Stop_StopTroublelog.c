/*!
 * @skip   $Id$
 * @file   m_lg_Stop_StopTroublelog.c
...
VOID m_lg_Stop_StopTroublelog( UCHAR *a_rcv_adr, USHORT a_rec_typ )
{
    CMT_TSKIF_CPRIRCV_REOBLOGSTPREQ *a_parm_p = NULL ; /* RE trouble log get stop request */
    UINT                            sigchk_rslt; /* result of checking signal */
    
    cm_Assert(D_RRH_LOG_AST_LV_ENTER, 0, "[m_lg_Stop_StopTroublelog] enter") ;
    ...
    cm_Assert( D_RRH_LOG_AST_LV_RETURN, 0, "[m_lg_Stop_StopTroublelog] return") ;
#ifdef FHM_TROUBLELOG_DEBUG_FOR_IT1
    printf( "[%d]%s end\n", __LINE__, __FUNCTION__);
#endif
