/*!
 * @skip   $Id$
 * @file   m_dl_fhm_fsndcmprsp_waiting_syspinf_ntc.c
...
/*!
 *  @brief  the processing for ファイル情報報告通知(対象データ：シスパラ), status: ファイルデータ送信完了報告応答(FW)待ち
 *  @note   
...
VOID    m_dl_fhm_fsndcmprsp_waiting_syspinf_ntc(VOID* bufp, USHORT cpri_no)
{
	INT			llErrcd;
	UINT		lwTimerId;

	cm_Assert(CMD_ASL_NOLEVEL, cpri_no, "[m_dl_fhm_fsndcmprsp_waiting_syspinf_ntc] ENTER");
