/*!
 * @skip   $Id$
 * @file   m_dl_rl_ufsnd_waiting_fw_stop_ind.c
...
/*!
 *  @brief  the processing for "REファイル更新中止指示" receiving at the 運用中ファイルデータ送信(FW)待ち REC state.
 *  @note   
...
VOID m_dl_rl_ufsnd_waiting_fw_stop_ind(VOID* bufp, USHORT cpri_no)
{
	USHORT  							lwTimerId;
	INT									ldwerrcd;
	UINT								ldwResult;
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rl_ufsnd_waiting_fw_stop_ind] ENTER" );
	...
	{
		cm_Assert( CMD_ASL_DBGLOW, ldwResult, "ファームウェア送信確認タイマ停止 Stop NG" );
	}
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rl_ufsnd_waiting_fw_stop_ind] RETURN" );
