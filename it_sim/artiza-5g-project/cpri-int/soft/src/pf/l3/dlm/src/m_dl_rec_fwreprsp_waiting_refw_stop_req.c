/*!
 * @skip   $Id$
 * @file   m_dl_rec_fwreprsp_waiting_refw_stop_req.c
...
/*!
 *  @brief  the processing for "REファイル更新中止要求" receiving at theファームウェアファイル報告応答待ち RE state.
 *  @note   
...
VOID m_dl_rec_fwreprsp_waiting_refw_stop_req(VOID* bufp, USHORT cpri_no)
{
	USHORT						lwTimerId;
	INT							ldwerrcd;
	UINT						ldwResult;
	
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_fwreprsp_waiting_refw_stop_req] ENTER" );
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_fwreprsp_waiting_refw_stop_req] RETURN" );
	return ;
}
