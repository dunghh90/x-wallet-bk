/*!
 * @skip   $Id$
 * @file   m_dl_rl_finfrsp_waiting_ufwrep_req.c
...
/*!
 *  @brief  the processing for "運用中ファームウェアファイル報告要求" receiving at the "REC主導ファイルデータ転送先決定待ち"
...
VOID m_dl_rl_finfrsp_waiting_ufwrep_req(VOID* bufp, USHORT cpri_no)
{
	...
	cm_Assert(CMD_ASL_NOLEVEL, cpri_no, "[m_dl_rl_finfrsp_waiting_ufwrep_req] ENTER");
	...
	if(ldwret_breq != CMD_RES_OK)
	{
		cm_MAbort(CMD_ALMCD_BUFGET,
					"m_dl_rl_finfrsp_waiting_ufwrep_req",
	...
	else
	{
		...
		BPF_COM_LOG_ASSERT_DWL(D_RRH_LOG_AST_LV_INFO, "ufwrrsp2:Num=%d, %x=V%02d%02d", 
