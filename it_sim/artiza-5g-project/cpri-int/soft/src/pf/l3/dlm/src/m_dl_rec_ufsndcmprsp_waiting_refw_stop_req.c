/*!
 * @skip   $Id$
 * @file   m_dl_rec_ufsndcmprsp_waiting_refw_stop_req.c
 ...
 *  @return Void.
 *  @retval -
 *  @Bug_No M-RRU-ZSYS-01852   
 *  @date   2015/08/28 FPT)DuongCD Create
 *  @date   2015/09/22 FPT)DuongCD IT update
 *  @date   2015/09/25 TDIPS)Lay M-RRU-ZSYS-01852 remove check code
 *  @date   2015/10/13 TDIPS)sasaki Update MKレビュー指摘No.289対応
 */
VOID m_dl_rec_ufsndcmprsp_waiting_refw_stop_req(VOID* bufp, USHORT cpri_no)
{
	USHORT						lwTimerId;
	INT							ldwerrcd;
	UINT						ldwResult;
	
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_ufsndcmprsp_waiting_refw_stop_req] ENTER" );
	...
	{
		cm_Assert( CMD_ASL_DBGLOW, ldwResult, "ファームウェア送信完了確認(更新用)タイマ Stop NG" );
	}
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_ufsndcmprsp_waiting_refw_stop_req] RETURN" );
	return ;
}
