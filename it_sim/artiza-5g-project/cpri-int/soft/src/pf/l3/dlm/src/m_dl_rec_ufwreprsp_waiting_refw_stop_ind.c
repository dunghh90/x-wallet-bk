/*!
 * @skip   $Id$
 * @file   m_dl_rec_ufwreprsp_waiting_refw_stop_ind.c
...
/*!
 *  @brief  the processing for "REファイル更新中止指示" receiving at the 運用中ファームウェアファイル報告応答待ち RE state.
 *  @note   
...
VOID m_dl_rec_ufwreprsp_waiting_refw_stop_ind(VOID* bufp, USHORT cpri_no)
{
	USHORT  							lwTimerId;
	INT									ldwerrcd;
	UINT								ldwResult;
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_ufwreprsp_waiting_refw_stop_ind] ENTER" );
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_ufwreprsp_waiting_refw_stop_ind] RETURN" );
	return ;
}
