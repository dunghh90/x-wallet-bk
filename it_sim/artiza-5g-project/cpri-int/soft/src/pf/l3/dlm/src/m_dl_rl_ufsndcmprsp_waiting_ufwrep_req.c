/*!
 * @skip   $Id$
 * @file   m_dl_rl_ufsndcmprsp_waiting_ufwrep_req.c
...
/*!
 *  @brief  the processing for "運用中ファームウェアファイル報告要求" receiving at the "REC主導運用中ファイルデータ送信完了報告応答(REC←FHM) 送信待ち"
...
VOID m_dl_rl_ufsndcmprsp_waiting_ufwrep_req(VOID* bufp, USHORT cpri_no)
{
	...
	cm_Assert(CMD_ASL_NOLEVEL, cpri_no, "[m_dl_rl_ufsndcmprsp_waiting_ufwrep_req] ENTER");
	...
	if(ldwret_breq != CMD_RES_OK)
	{
		cm_MAbort(CMD_ALMCD_BUFGET,
					"m_dl_rl_ufsndcmprsp_waiting_ufwrep_req",
	...
	else
	{
		...
		for(lwLoop = 0; lwLoop < D_RRH_CPRINO_RE_MAX; lwLoop++)
		{
			...
			{
				pRsp->cpridat_ufirmfileres2.firmfile[pRsp->cpridat_ufirmfileres2.verinf_num].makername = gt_dlw_refw_tbl[lwLoop].mk_jdg;
				pRsp->cpridat_ufirmfileres2.firmfile[pRsp->cpridat_ufirmfileres2.verinf_num].nes_mjr_ver = gt_dlw_refw_tbl[lwLoop].mjr_ver;
				pRsp->cpridat_ufirmfileres2.firmfile[pRsp->cpridat_ufirmfileres2.verinf_num].nes_mir_ver = gt_dlw_refw_tbl[lwLoop].min_ver;
				pRsp->cpridat_ufirmfileres2.firmfile[pRsp->cpridat_ufirmfileres2.verinf_num].cur_mjr_ver = gt_dlw_refw_tbl[lwLoop].usmjr_ver;
				pRsp->cpridat_ufirmfileres2.firmfile[pRsp->cpridat_ufirmfileres2.verinf_num].cur_mir_ver = gt_dlw_refw_tbl[lwLoop].usmin_ver;
				pRsp->cpridat_ufirmfileres2.verinf_num++;
			}
		}
