/*!
 * @skip   $Id$
 * @file   m_dl_rec_ufsnd_waiting_fw_stop_req.c
...
/*!
 *  @brief  the processing for "REファイル更新中止要求" receiving at the 運用中ファイルデータ送信中(FW) RE state.
 *  @note   
...
VOID m_dl_rec_ufsnd_waiting_fw_stop_req(VOID* bufp, USHORT cpri_no)
{
	
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_ufsnd_waiting_fw_stop_req] ENTER" );
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0,	"[m_dl_rec_ufsnd_waiting_fw_stop_req] RETURN" );
	return ;
}
