/*!
 * @skip   $Id$
 * @file   m_dl_rl_syspsndcmprsp_waiting_finf_ntc_ng.c
...
/*!
 *  @brief  the processing for "ファイル情報報告通知" receiving at the ファイルデータ送信完了報告応答(シスパラ)集約中 REC state.
 *  @note   
...
VOID m_dl_rl_syspsndcmprsp_waiting_finf_ntc_ng(VOID* bufp, USHORT cpri_no)
{
	UINT	ldwsigchk;
	INT		ldwerrcd;
	...
	cm_Assert(CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_syspsndcmprsp_waiting_finf_ntc_ng] ENTER");
	...
	/****************************************************************************/
	/* システムパラメータ送信完了確認タイマ停止 */
	(VOID)cm_TStop((CMD_TIMID_SYSPRM_SNDFIN + D_RRH_CPRINO_RE_MAX * lw_systype) , &ldwerrcd);
	cm_Assert(CMD_ASL_DBGLOW, CMD_NUM0, "[m_dl_rl_syspsndcmprsp_waiting_finf_ntc_ng] Timer Stop");			
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_syspsndcmprsp_waiting_finf_ntc_ng] RETURN" );
	return ;
}
