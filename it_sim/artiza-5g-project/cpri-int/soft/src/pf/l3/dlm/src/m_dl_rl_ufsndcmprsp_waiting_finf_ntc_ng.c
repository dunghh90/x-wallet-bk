/*!
 * @skip   $Id$
 * @file   m_dl_rl_ufsndcmprsp_waiting_finf_ntc_ng.c
...
/*!
 *  @brief  the processing for "ファイル情報報告通知" receiving at the 運用中ファイルデータ送信完了報告応答(REC←FHM) 送信待ち REC state.
 *  @note   
 ...
 */
VOID m_dl_rl_ufsndcmprsp_waiting_finf_ntc_ng(VOID* bufp, USHORT cpri_no)
{
	UINT	ldwResult;
	UINT	ldwsigchk;
	INT		ldwerrcd;
	...
	cm_Assert(CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_ufsndcmprsp_waiting_finf_ntc_ng] ENTER");
...
//			if(gt_dlw_refw_tbl[lwLoop].use_timer != DLD_RERETIM_NONE)
			{
				ldwResult = cm_TStop((CMD_TIMID_UFIRMFILE_SNDFIN + lwLoop), &ldwerrcd);
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_ufsndcmprsp_waiting_finf_ntc_ng] RETURN" );
	return ;
}
