/*!
 * @skip   $Id$
 * @file   m_dl_fhm_fwreprsp_waiting_syspinf_ntc.c
...
/*!
 *  @brief  the processing for ファイル情報報告通知(対象データ：シスパラ), status: ファームウェアファイル報告応答待ち.
 *  @note   
...
VOID    m_dl_fhm_fwreprsp_waiting_syspinf_ntc(VOID* bufp, USHORT cpri_no)
{
	INT			llErrcd;
	UINT		lwTimerId;
	UINT		lwSysType = ((CMT_TSKIF_HEAD *)bufp)->uiSrcPQueueID;

	cm_Assert(CMD_ASL_NOLEVEL, cpri_no, "[m_dl_fhm_fwreprsp_waiting_syspinf_ntc] ENTER");
