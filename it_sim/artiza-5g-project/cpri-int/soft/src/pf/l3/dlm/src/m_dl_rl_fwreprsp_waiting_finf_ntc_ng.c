/*!
 * @skip   $Id$
 * @file   m_dl_rl_fwreprsp_waiting_finf_ntc_ng.c
...
/*!
 *  @brief  the processing for "ファイル情報報告通知" receiving at the ファームウェア報告応答集約中 REC state.
 *  @note   
 ...
 *  @date   2015/11/25 TDIPS)sasaki M-RRU-ZSYS-02006 Update IT2問処No.193対処 対RE状態の初期化処理を無条件に実行するよう修正
 *  @date   2015/11/25 TDIPS)sasaki M-RRU-ZSYS-02007 Update IT2問処No.194対処 使用中タイマ情報を無視してファームウェアファイル報告確認(更新用)タイマ停止処理を実行
 */
VOID m_dl_rl_fwreprsp_waiting_finf_ntc_ng(VOID* bufp, USHORT cpri_no)
{
	UINT	ldwResult;
	UINT	ldwsigchk;
	INT		ldwerrcd;
	...
	cm_Assert(CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_fwreprsp_waiting_finf_ntc_ng] ENTER");
...
//		if(gt_dlw_refw_tbl[lwLoop].use_timer != DLD_RERETIM_NONE)
		{
			ldwResult = cm_TStop((CMD_TIMID_FIRMFILE_REP + lwLoop), &ldwerrcd);
			if(ldwResult != CMD_RES_OK)
			{
				cm_Assert(CMD_ASL_DBGLOW, ldwResult, "ファームウェアファイル報告確認タイマ停止 NG");
			}
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_fwreprsp_waiting_finf_ntc_ng] RETURN" );
	return ;
}
