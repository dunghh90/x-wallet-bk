/*!
 * @skip   $Id$
 * @file   m_dl_rl_ufinfrsp_waiting_finf_ntc_ng.c
...
/*!
 *  @brief  the processing for "ファイル情報報告通知" receiving at the 運用中ファイルデータ転送先決定待ち REC state.
 *  @note   
 ...
 */
VOID m_dl_rl_ufinfrsp_waiting_finf_ntc_ng(VOID* bufp, USHORT cpri_no)
{
	UINT	ldwsigchk;
	INT		ldwerrcd;
	...
	cm_Assert(CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_ufinfrsp_waiting_finf_ntc_ng] ENTER");
...
//	if(DLD_RECRETIM_SND_UPDATE == gt_dlw_rec_tbl.use_timer)
	{
		...
		cm_Assert(CMD_ASL_DBGLOW, CMD_NUM0, "[m_dl_rl_ufinfrsp_waiting_finf_ntc_ng] Timer Stop");			
	...
	cm_Assert( CMD_ASL_NOLEVEL, CMD_NUM0, "[m_dl_rl_ufinfrsp_waiting_finf_ntc_ng] RETURN" );
	return ;
}
