/*!
 * @skip   $Id$
 * @file   m_dl_rl_fsnd_waiting_fwrep_req.c
...
/*!
 *  @brief  the processing for "ファームウェアファイル報告要求" receiving at the "REC主導ファイルデータ送信(FW)待ち".
 *  @note   
...
VOID m_dl_rl_fsnd_waiting_fwrep_req(VOID* bufp, USHORT cpri_no)
{
	...
	cm_Assert(CMD_ASL_NOLEVEL, cpri_no, "[m_dl_rl_fsnd_waiting_fwrep_req] ENTER");
	...
	if(ldwret_breq != CMD_RES_OK)
	{
		cm_MAbort(CMD_ALMCD_BUFGET, 
					"m_dl_rl_fsnd_waiting_fwrep_req",
