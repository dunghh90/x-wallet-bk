 ...
 *				-# 受信メッセージ内イベント番号異常又はスレッド状態異常の場合、アボート処理を実施する(f_pfm_abort())
 *				-# 受信メッセージ処理異常の場合、Assertログ処理を実施する(f_pfm_assert))
...
INT	f_pfm_anlz(VOID*	msgP)
{
	...
	switch( ((T_SYS_COMMON_THDIF *)msgP)->uiEventNo ) {
		case	D_SYS_MSGID_MSI_RCV_IND:								/* MSI受信通知			*/
			evtNo = E_PFM_EVT_MSI_RCV_IND;
			break;

		case	D_SYS_MSGID_PFM_1S_TIMEOUT_NTF:							/* タイムアウト通知		*/
			evtNo = E_PFM_EVT_1S_TIMEOUT_NTF;
			break;

		case	D_SYS_MSGID_PFM_LOG_TIMEOUT_NTF:							/* タイムアウト通知		*/
			evtNo = E_PFM_EVT_LOG_TIMEOUT_NTF;
			break;
	...
	/* Thread状態チェック	*/
	if( f_pfmw_thdInfo.thdState >= E_PFM_EVT_MAX ) {

		/* Assertログ出力	*/
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_WARNING, 					/* Assertレベル			*/
							"Thread Status Error, sta=%d",				/* message				*/
							f_pfmw_thdInfo.thdState );					/* Information			*/
	...
	/* マトリクス関数コール	*/
	rtn = (*(f_pfmr_callSetRell_mtxtbl[f_pfmw_thdInfo.thdState][evtNo].eventProcess_addr))(msgP);
	...
	if( rtn != D_SYS_OK ) {
		...
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_WARNING, 					/* Assertレベル			*/
							...
							rtn, f_pfmw_thdInfo.thdState, evtNo );		/* Information			*/
	}
	return	rtn;
}
