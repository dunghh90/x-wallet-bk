 ...
 * @date		2013/11/23 ALPHA)横山 Create
 */
INT f_log_send_EventLogReq( UINT	kind )
{
	T_SYS_FRSP_FLSLOGWR_IND		*sndMsg_FlsLogWrInd;							/* 送信メッセージ(Flash Log WR)		*/
	INT							ret;
	INT							errcd;
	
	cmn_com_assert_th_get(f_logw_assert_p, D_RRH_LOG_AST_LV_ENTER, "[f_log_send_EventLogReq] ENTER" );
	...
	sndMsg_FlsLogWrInd->head.uiDstPQueueID	= D_RRH_PROCQUE_F_PF;					/* 返信先PQID						*/
	sndMsg_FlsLogWrInd->head.uiDstTQueueID	= D_SYS_THDQID_PF_FRQ;				/* 送信先TQID						*/
	sndMsg_FlsLogWrInd->head.uiSrcPQueueID	= D_RRH_PROCQUE_F_PF;					/* 送信元PQID						*/
	sndMsg_FlsLogWrInd->head.uiSrcTQueueID	= D_SYS_THDQID_PF_LOG;				/* 送信元TQID						*/
	
	/* Flashへ退避したログの削除依頼を送付		*/
	sndMsg_FlsLogWrInd->eraseWriteKind	= kind;
	
	ret = BPF_RU_IPCM_MSGQ_SEND(	D_SYS_THDQID_PF_FRQ,						/* 送信MSGキューID					*/
	...
	cmn_com_assert_th_get(f_logw_assert_p, D_RRH_LOG_AST_LV_RETURN, "[f_log_send_EventLogReq] RETURN" );
	return D_RRH_OK;
}
