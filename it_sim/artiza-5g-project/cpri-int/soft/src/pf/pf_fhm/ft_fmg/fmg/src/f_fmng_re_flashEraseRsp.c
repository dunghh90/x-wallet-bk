/*!
 * @skip  $ld:$
 * @file  f_fmng_re_flashEraseRsp.c
 * @brief Reception of RE-FLASH ERASE response from FRWRE thread
 ...
 * @date  2015/08/21 TDIPS)sasaki RE-FLASH ERASE結果の保存処理を追加
...
VOID f_fmng_re_flashEraseRsp( VOID *pRcvMsg )
{
	cmn_com_assert_th_get(f_fmngw_assert_p, D_RRH_LOG_AST_LV_ENTER, "[f_fmng_re_flashEraseRsp] ENTER" );
	
	f_fmng_flashDelReFwInfo(pRcvMsg);
	...
	f_fmngw_re_fileUpdResult = ((T_SYS_FLSERASE_RSP *)pRcvMsg)->result;
	f_fmngw_re_thrdState = D_FMNG_THRDSTA_RE_FWINFO_SAVE;

	cmn_com_assert_th_get(f_fmngw_assert_p, D_RRH_LOG_AST_LV_RETURN, "[f_fmng_re_flashEraseRsp] RETURN to FWINFO_SAVE" );
	return;
}
