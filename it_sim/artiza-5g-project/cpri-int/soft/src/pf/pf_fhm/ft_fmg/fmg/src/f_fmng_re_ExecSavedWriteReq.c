/*!
 * @skip  $ld:$
 * @file  f_fmng_re_ExecSavedWriteReq.c
 * @brief Send Saved WRITE Request to FRWRE thread
...
VOID f_fmng_re_ExecSavedWriteReq( VOID *pRcvMsg )
{
	cmn_com_assert_th_get(f_fmngw_assert_p, D_RRH_LOG_AST_LV_ENTER, "[f_fmng_re_ExecSavedWriteReq] ENTER" );				

	f_com_msgQSendFHM( D_SYS_THDQID_PF_FRQRE,
					(CHAR*)&f_fmngw_flashWaitWrtReq_re,
					sizeof(f_fmngw_flashWaitWrtReq_re), 
					D_SYS_THDQID_PF_FRMG, 
					f_fmngw_re_thrdState, 
					D_SYS_MSGID_FLASH_RE_WRITE_REQ );

	/* Set received api kind	*/
	f_fmngw_re_apikind = D_FMNG_API_WRT;

	/* Change thread state into RE-FLASH READ/WRITE		*/
	f_fmngw_re_thrdState = D_FMNG_THRDSTA_RE_FLSHRW;

	cmn_com_assert_th_get(f_fmngw_assert_p, D_RRH_LOG_AST_LV_RETURN, "[f_fmng_re_ExecSavedWriteReq] RETURN to FLSHRW" );
	return;
}
