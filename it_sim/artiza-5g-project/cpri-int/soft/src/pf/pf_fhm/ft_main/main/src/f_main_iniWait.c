...
UINT f_main_iniWait(VOID* rcvMsg_p)
{
	...
	sndMsg->head.uiDstPQueueID	= D_RRH_PROCQUE_F_PF;								/* 送信先Process Queue ID		*/
	sndMsg->head.uiDstTQueueID	= D_SYS_THDQID_PF_RSV;							/* 送信先Thread Queue ID		*/
	sndMsg->head.uiSrcPQueueID	= D_RRH_PROCQUE_F_PF;								/* 送信元Process Queue ID		*/
	sndMsg->head.uiSrcTQueueID	= D_SYS_THDQID_PF_F_MAIN;							/* 送信元Thread Queue ID		*/
...
VOID f_main_troubelelog_send(INT log_status)
{
	    ...
	    (log_status == D_RRH_SIGUSR_OPT_RUNLOG_FIN_PF_3G ) )
	{
		...
		/* ヘッダ部設定	*/
		sndRunlogMsg->head.uiDstPQueueID	= D_RRH_PROCQUE_F_PF;						/* 返信先PQID						*/
		sndRunlogMsg->head.uiDstTQueueID	= D_SYS_THDQID_PF_LOG;					/* 送信先TQID						*/
		sndRunlogMsg->head.uiSrcPQueueID	= D_RRH_PROCQUE_F_PF;						/* 送信元PQID						*/
		sndRunlogMsg->head.uiSrcTQueueID	= D_SYS_THDQID_PF_F_MAIN;					/* 送信元TQID						*/
	...
	else
	{
		...
		/* ヘッダ部設定	*/
		sndTroublelogMsg->head.uiDstPQueueID	= D_RRH_PROCQUE_F_PF;					/* 返信先PQID						*/
		sndTroublelogMsg->head.uiDstTQueueID	= D_SYS_THDQID_PF_LOG;				/* 送信先TQID						*/
		sndTroublelogMsg->head.uiSrcPQueueID	= D_RRH_PROCQUE_F_PF;					/* 送信元PQID						*/
		sndTroublelogMsg->head.uiSrcTQueueID	= D_SYS_THDQID_PF_F_MAIN;				/* 送信元TQID						*/
...
VOID f_main_flash_write_req_send(INT log_status)
{
	...
	/* ヘッダ部設定	*/
	sndTroubleWriteMsg->head.uiDstPQueueID	= D_RRH_PROCQUE_F_PF;						/* 返信先PQID						*/
	sndTroubleWriteMsg->head.uiDstTQueueID	= D_SYS_THDQID_PF_FRQ;					/* 送信先TQID						*/
	sndTroubleWriteMsg->head.uiSrcPQueueID	= D_RRH_PROCQUE_F_PF;						/* 送信元PQID						*/
	sndTroubleWriteMsg->head.uiSrcTQueueID	= D_SYS_THDQID_PF_F_MAIN;					/* 送信元TQID						*/
