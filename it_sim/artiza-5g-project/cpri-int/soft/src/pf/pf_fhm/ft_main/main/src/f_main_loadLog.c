 ...
 * @date  2015/08/11 TDIPS) maruyama RE種別（f_comw_reKind）で配列の切り分けを取りやめ（データを直接セットする）
 *
...
T_MAIN_LOADLOG f_inir_loadLog[1][4] =
{
		/* face   ,  addr */
	{
		...
		{ D_RRH_QSPI_FLASH_FACE1, D_RRH_QSPI_FLASH_OFFSET_TROUBLE2 },        /* E_RRH_QSPI_FLASH_TROUBLELOG_FACE2 */
		{ D_RRH_QSPI_FLASH_FACE1, D_RRH_QSPI_FLASH_OFFSET_HEALTH   }         /* D_RRH_QSPI_FLASH_OFFSET_HEALTH */
	}
};
 ...
 *					- 再取得フラグがONの場合.
 *						-# 異常終了を返却し、処理終了.
 * @param		cyclic_log_face [in] サイクリックログ保存面
...
UINT f_main_loadLog_develop(UCHAR cyclic_log_face)
{
	...
	UINT					uiFileSize;
	UINT					uiKeyId;
	UINT					uiFaceState[ 2 ] = { D_RRH_EEP_CYCLIC_CLEAR_FLG0, D_RRH_EEP_CYCLIC_CLEAR_FLG1 };
	UCHAR					uiState;

	BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ENTER, "[f_main_loadLog_develop] ENTER" );
	...
	if( (cyclic_log_face == 0) || (cyclic_log_face == 1) )
	{
		...
		for( uiFaceCnt = 0; uiFaceCnt < 2; uiFaceCnt++ )
		{
			...
			{
				BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "EEPROM READ NG");
				break;
			}
			...
			{
				BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_ERROR, "Flash Log Info load NG. rtn_bpf=%08x", rtn_bpf);
				/* 逆面をTRY	*/
				cyclic_log_face = ~cyclic_log_face & 1;
				continue;
			}
			...
			{
				/* 正常なデータでない場合は展開しない	*/
				BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_ERROR, "File Size error. uiFileSize = %d", uiFileSize);
				/* 逆面をTRY	*/
				cyclic_log_face = ~cyclic_log_face & 1;
				continue;
			}
			...
			{
				/* 解凍に失敗した場合は展開しない	*/
				BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_ERROR, "Flash Log decompress NG. iRet = %d", iRet);
				/* 逆面をTRY	*/
				cyclic_log_face = ~cyclic_log_face & 1;
				continue;
			}
			...
			for( logCnt=0; logCnt<D_RRH_FLASH_LOG_MAX; logCnt++ )
			{
				...
				if(	(uiOffset + uiOutLen) > sizeof(f_compw_compOut) )
				{
					BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_ERROR, "Invalid Cyclic Log Size(%d,%d,0x%08x)", logCnt,uiKeyId,uiOutLen );
					break;
				}
				...
				if( ( uiKeyId != f_comr_flash_log_save_info_fhm[ logCnt ].shmid   ) ||
					( uiOutLen!= f_comr_flash_log_save_info_fhm[ logCnt ].logSize ) )
				{
					/* オフセット更新	*/
					uiOffset += uiOutLen;
					continue;
				...
				{
					BPF_COM_LOG_ASSERT(D_RRH_LOG_AST_LV_ERROR, "Shared memory address NG. rtn_bpf=%08x", rtn_bpf);
					continue;
				}
				/* 共有メモリへの展開を行う	*/
	...
	BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_RETURN, "[f_main_loadLog_develop] RETURN" );
 ...
 * @note		ログ出力展開処理を行う.
 *					-# ログ出力
 * @param		filename_info [in] file name information (障害ログ面情報)
 * @retval		D_SYS_OK	0:正常終了
 * @retval		D_SYS_NG	1:異常終了
 * @return		処理結果
 * @warning		N/A
...
UINT f_main_loadLog_outputFile(UCHAR filename_info)
{
	...
	INT						cnt;
	UINT					pattern;
	char					filename[4][128] = {"/var/log/trouble00.tar.gz",
...
UINT f_main_loadLog()
{
	...
	/************************************************************/
	/* Flash面情報取得 */
	rtn_bpf = BPF_HM_DEVC_EEPROM_READ(D_RRH_EEP_CYCLIC_LOG, &rData);
	if( rtn_bpf != BPF_HM_DEVC_COMPLETE )
	{
	...
	/************************************************/
	/* 障害ログ退避面情報取得 */
	rtn_bpf = BPF_HM_DEVC_EEPROM_READ(D_RRH_EEP_TROUBLE_LOG, &rData);
	if( rtn_bpf != BPF_HM_DEVC_COMPLETE )
	{
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, " EEPROM EVTLOG Side read NG. rtn_bpf=%08x", rtn_bpf);
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_RETURN, "[f_main_loadLog] RETURN" );
	...
	if(rData >= E_RRH_QSPI_FLASH_TROUBLELOG_FACE_MAX )
	{
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "EVTLOG Side range ERROR. rData=%08x", rData);
	}
	else
	{
