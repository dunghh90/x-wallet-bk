/*!
 * @skip    $Id$
 * @file    tm_lb_rell2link_rsp.c
 * @brief   Send response of release lapb link
 ...
 *  @retval -
 *  @date   2013/11/21 FFCS)hongj Create zynq4
 *  @date    2015.08.11 TDIPS)ikeda 17リンク対応
 */
 /********************************************************************************************************************/
void 	tm_lb_rell2link_rsp(USHORT linkno, UINT result, CHAR* errMsg)
{
	T_API_L2LINK_REL_RSP apiRsp;
	UINT copyLen;

	/* send response of release lapb link */
	memset(&apiRsp, 0, sizeof(apiRsp));

	if(lbw_lnk_rel_respQId[linkno] == CMD_NUM0)
	{
		cm_Assert( CMD_ASL_DBGLOW, lbw_lnk_rel_respQId[linkno],
				"[tm_lb_rell2link_rsp] There isn't release response queue Id." );
		return;
	}
	apiRsp.header.uiEventNo		= D_API_MSGID_L2_LINK_REL_RSP;		/**< Event	ID		   */
	apiRsp.header.uiSignalkind	= 0;								/**< Signal Kind	   */
	apiRsp.header.uiDstPQueueID	= lbw_lnk_rel_respQId[linkno];				/**< Destination P QID */
	...
	tm_lb_RunHisSet(LBD_RUNHIS_FLG_SND, CMD_TSKID_CPR, D_API_MSGID_L2_LINK_REL_RSP, (UINT *)&apiRsp);
	/* call send message func */
	l2_com_comMsgSnd(
			lbw_lnk_rel_respQId[linkno],			/* Thread queue ID		   */
			D_API_MSGID_L2_LINK_REL_RSP,
