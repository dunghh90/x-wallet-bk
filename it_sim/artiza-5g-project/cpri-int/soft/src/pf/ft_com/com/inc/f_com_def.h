 ...
 * @file  f_com_def.h
 * @brief 共通関数定数定義ヘッダ
 * @date  2008/08/08 ALPHA) 高橋 Create
 * @date  2008/11/06 FFCS) limin Update LOS/LOF control bit 対応
 * @date  2008/11/06 FFCS) limin M-RRU-ZSYS-00045 treating
 * @date  2008/11/11 ALPHA) 戸塚 M-RRU-ZSYS-00056 対処
 * @date  2008/12/05 ALPHA) 宮永 最新Flashメモリマップ対応
 * @date  2008/12/09 ALPHA) 宮永 障害ログ収集項目追加
 * @date  2008/12/22 ALPHA) 藤井 M-RRU-ZSYS-00162 対処
 * @date  2009/01/13 ALPHA) 宮永 DL仕様変更対応
 * @date  2009/01/14 ALPHA) 的場 LOG Save用テーブル情報テーブル ハードソフトインタフェース案v7_5対応
 * @date  2009/01/15 ALPHA) 宮永 M-RRU-ZSYS-00198 対処
 * @date  2009/01/20 ALPHA) 宮永 Log収集方法改善
 * @date  2009/01/26 ALPHA) 宮永 不要テーブル削除に伴い関連define削除
 * @date  2009/01/28 FFCS) limin EEPROM check improvement for new DL spec
 * @date  2010/01/26 ALPHA) 横山 LTE-RRH対応
 * @date  2010/02/26 FFCS) Linlj IIC data access semaphore type added
 * @date  2010/04/22 ALPHA) 藤井 M-RRU-RRHFT-00033 対処
 * @date  2010/05/20 ALPHA) 星野 MT-RRH対応
 * @date  2010/06/02 ALPHA) 横山 M-RRU-ZSYS-00369  対処
 * @date  2010/07/05 ALPHA) 藤井 M-RRU-ZSYS-00400  対処(spell check)
 * @date  2010/07/13 ALPHA) 横山 M-RRU-ZSYS-00411  対処
 * @date  2010/07/22 ALPHA) 藤井 M-RRU-ZSYS-00420  対処
 * @date  2010/09/14 ALPHA) 藤井 M-RRU-ZSYS-00450  対処
 * @date  2010/09/29 ALPHA) 横山 AISG対応
 * @date  2010/11/22 ALPHA) 横山 M-RRU-ZSYS-00530  対処
 * @date  2010/11/25 FFCS) Shihzh AISG log save table
 * @date  2011/04/11 ALPHA) 横山 LTE Only Mode対応
 * @date  2011/08/16 ALPHA) 横山 M-RRU-ZSYS-00690対応
 * @date  2012/10/22 COMAS) 中島 MTCD対応
 * @date  2011/12/18 ALPHA) 藤井 M-RRU-ZSYS-00922対応
 * @date  2013/11/18 ALPHA) 加藤 ZYNQ化対応
 * @date  2015/06/22 ALPHA) 上田 TDD-SRE(Zynq)対応
...
/*!
 * @name Register dump情報種別
 * @note 
 * @{
 */
#define D_COM_REG_DMP_KIND_ALMHIS       0            /*!< Alamr History Log収集時        */
#define D_COM_REG_DMP_KIND_RUNLOG       1            /*!< 運用ログ収集時(障害取得マクロ) */
/* @} */
...
/*!
 * @name apd initialization related definition
 * @note apd initialization related definition
 * @{
 */
#define	D_COM_INIT_CRCPOLY1				0x1021			/*!< CRC16計算用	x^{16}+x^{12}+x^5+1			*/
/* @} */
...
/*!
 * @name regster log address(periodic) for zynq slc
 * @note regster log address(periodic) for zynq slc definition
 * @{
 */
enum
{
	E_COM_REGIDX_SLC_CPU_SV0INF,							/* SV状態表示 A0/A1						*/
	E_COM_REGIDX_SLC_CPU_SV1INF,							/* SV状態表示 A2/A3						*/
	E_COM_REGIDX_SLC_CPU_SV2INF,							/* SV状態表示 A4/A5						*/
	E_COM_REGIDX_SLC_CPU_SV3INF,							/* SV状態表示 A6/A7						*/
	E_COM_REGIDX_SLC_CPU_SV4INF,							/* SV状態表示 A8/A9						*/
	E_COM_REGIDX_SLC_FPGA_CSTATE,							/* CPRI ステート表示					*/
	E_COM_REGIDX_SLC_MAX									/* regster log 数(ZynqSLC)				*/
};
/* @} */
...
#define D_COM_MAX_SYSNUM			2	/**< Max connected system number for shared-RE */
#define D_COM_SYS_3G				0	/**< connected system is 3G */
#define D_COM_SYS_S3G				1	/**< connected system is S3G */
#define D_COM_SYS_BOTH_3G_S3G		2	/**< connected system is both 3G and S3G */
...
#define D_COM_CONN_3G_SINGLE		0	/**< 接続構成 is 3G単独 */
#define D_COM_CONN_S3G_SINGLE		1	/**< 接続構成 is S3G単独 */
#define D_COM_CONN_CPRI_SHARED		2	/**< 接続構成 is CPRI共用 */
 ...
 * @{  
 */
#define D_COM_ALMDETECT_SW		0x0001
#define D_COM_ALMDETECT_HW		0x0002
#define D_COM_HARDALM_MAX			48
/* @} */

/* ======================================================================== */
/* CPRI stateF 制御レジスタ								(R/W)	*/
/* ======================================================================== */
#define D_COM_T4_TIMER			0x0064	/**< T4 timer default value (10s) */
...
enum
{
	D_COM_LINE_BITRATE_UNKNOWN = 0,
	D_COM_LINE_BITRATE_24G,						/* 2.4G 固定				*/
	D_COM_LINE_BITRATE_49G,						/* 4.9G						*/
	D_COM_LINE_BITRATE_98G						/* 9.8G						*/
};
 ...
 * @{  
 */
#define D_COM_FWVATT_CLK					0x000AAA00			/**< クロック設定			*/
#define D_COM_FWVATT_EN						0x000000C0			/**< イネーブル設定			*/
#define D_COM_FWVATT_DATA_NUM				4					/**< FWVATTデータ数			*/
#define D_COM_FWVATT_DATA_NUM_RRE			2					/**< FWVATTデータ数(RRE用)	*/
/* @} */
 ...
 * @{  
 */
#define D_COM_PADAC_CLK						0x00FFFF00			/**< クロック設定			*/
#define D_COM_PADAC_EN						0xFF0000FF			/**< イネーブル設定			*/
#define D_COM_PADAC_DATA_NUM				8					/**< PADACデータ数			*/
/* @} */
 ...
 * @{  
 */
#define D_COM_VSS_DPD_MASK					0x00010C0C			/**< DPD部 マスクデータ				*/
...
enum
{
	E_COM_SFP_TYPE_ABNORMAL = 0,	/* 確認不可(異常ケース)		*/	
	E_COM_SFP_TYPE_UNSUPT,			/* 未サポート				*/
	E_COM_SFP_TYPE_C,				/* Cタイプ					*/
	E_COM_SFP_TYPE_D,				/* Dタイプ					*/
	E_COM_SFP_TYPE_SHORT_1385,		/* 短距離用1385				*/
	E_COM_SFP_TYPE_SHORT_1386,		/* 短距離用1386				*/
	E_COM_SFP_TYPE_NUM				/* SFPタイプ数				*/
};
/* @} */
 ...
 * @{
 */
#define D_COM_SFP_MON_STATE_UNMOUNT			0					/*!< SFP Monitor状態：Unmount			*/
#define D_COM_SFP_MON_STATE_MOUNT			1					/*!< SFP Monitor状態：Mount				*/
#define D_COM_SFP_MON_STATE_MOUNT_CHG		2					/*!< SFP Monitor状態：Unmount→Mount	*/
#define D_COM_SFP_MON_STATE_INIT			0xFFFFFFFF			/*!< SFP Monitor状態：監視前			*/
/* @} */
...
enum
{
	E_COM_RRH_KIND_5GLLS_PROT = 0,	/* 5G-LLS(Proto)			*/
	E_COM_RRH_KIND_MAX				/* 装置種別数				*/
};
/* @} */
