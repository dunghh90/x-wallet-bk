/*!
 * @skip  $ld:$
* @file  f_rfcp_rxoff_comfirm.c(base: f_rfcp_rxon_comfirm.c)
 * @brief pf_rfcpスレッド RX-OFF(M-Plane)応答受信処理
...
/****************************************************************************************************************************/
/*!
 * @brief		f_rfcp_rxoff_comfirm
 * @note		RX-OFF(M-Plane)応答受信時の処理を行う
 *				-# パラメータチェック実施
 *				-# 処理実行中状態を返却する
...
{
	T_RFP_MPL_RX_OFF_SETTING_CFM*		rcvMsg = (T_RFP_MPL_RX_OFF_SETTING_CFM*)msgP ;

    BPF_COM_PLOG_TRACE_RFIC( D_RRH_LOG_AST_LV_ENTER , "f_rfcp_rxoff_comfirm called");
	...
	if(f_upcnf_latest_msi_reqid[rcvMsg->ant][rcvMsg->car][D_RFCP_UPCNF_STSRX] == rcvMsg->requestId) {
		/* 最新の要求に対する応答の場合 */
		/* 状態の変更 */
		f_upcnf_state[rcvMsg->ant][rcvMsg->car][D_RFCP_UPCNF_STSRX] = E_RFCP_UPCNF_STS_DISABLE ;
