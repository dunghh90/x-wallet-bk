...
char	t_dpda_txoff_err[32] = "failed get send msg addr.";

/*!
 * @brief 関数機能概要:TX-OFF(M-Plane)設定
 * @note  関数処理内容.
 * @param  *msgP        [in]void            受信メッセージ
 * @return int
 * @date 2018/11/22 KCN)上本 Create
 */
int f_dpda_act_txoff_request(VOID* msgP)
{
    BPF_COM_PLOG_TRACE_RFIC( D_RRH_LOG_AST_LV_ENTER , "f_dpda_act_txoff_request called");

    T_RFP_MPL_TX_OFF_SETTING_REQ*	msgOFF = NULL;
    T_RFP_MPL_TX_OFF_SETTING_CFM*	sndMsg;
    ...
    msgOFF       = (T_RFP_MPL_TX_OFF_SETTING_REQ*)msgP;
    uiAntNum     = msgOFF->body.tx_array_car_t.ant ;
    uiCarrier    = msgOFF->body.tx_array_car_t.car ;
    uiCrCentFreq = msgOFF->body.tx_array_car_t.abs_freq_center;
    uiUnitType   = f_dpdaw_reKind;


	/* duコマンド用 */
	BPF_COM_CMD_SET_TXENDP(uiCarrier,uiAntNum,D_RRH_OFF,0,0,0,D_DPDA_CELL_INVALID,0,0);
	BPF_COM_CMD_SET_TXCC(uiCarrier ,uiAntNum ,0, 0, 0);
	...
	{
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txoff_request error=0x%08x", ret);
		/* 通信用共有メモリを取得する	*/
		ret = BPF_RU_IPCM_PROCMSG_ADDRGET(	E_BPF_RU_IPCM_BUFF_KIND_TASK,			/* Buffer種別						*/
											sizeof(T_RFP_MPL_TX_OFF_SETTING_CFM),	/* Size								*/
											(VOID **)&sndMsg,						/* sndMsg								*/
											&errcd );
		if( ret != D_SYS_OK )
		{
			BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txoff_request buffer error=0x%08x", ret);
		}
		else
		{
			sndMsg->thdif.uiEventNo		= D_SYS_MSGID_TXOFF_CFM ;		/* TX-OFF(M-Plane)応答				*/
			...
			memcpy( &( sndMsg->msgErr.msgErr.errorDescription ) , t_dpda_txoff_err , D_DPDA_UPCNF_ERR_MSG_SIZE ) ;

			/* TX-OFF(M-Plane)応答を送信する */
			ret = BPF_RU_IPCM_MSGQ_SEND(	D_SYS_THDQID_PF_RFA ,		/* 送信先Thread Queue ID			*/
											(void *)sndMsg );			/* 送信Message						*/

			if( ret != D_SYS_OK )
			{
				BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txoff_request failed send message. %d", ret);
			}
		}
