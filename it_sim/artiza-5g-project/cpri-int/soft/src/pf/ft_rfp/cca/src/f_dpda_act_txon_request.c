...
char	t_dpda_txon_err[32]			= "failed get send msg addr.";
char	t_dpda_txon_err_input[32]	= "(SCS)Input parameter Error.";

/*!
 * @brief 関数機能概要:TX-ON(M-Plane)設定
 * @note  関数処理内容.
 ...
 */
int f_dpda_act_txon_request( void* msgP )
{
    BPF_COM_PLOG_TRACE_RFIC( D_RRH_LOG_AST_LV_ENTER , "f_dpda_act_txon_request called");

    T_RFP_MPL_TX_ON_SETTING_REQ*	msgON = NULL;	/* TX-ONメッセージ		*/
	T_RFP_MPL_TX_ON_SETTING_CFM*	sndMsg ;
    ...
    msgON        = (T_RFP_MPL_TX_ON_SETTING_REQ*)msgP;
    uiAntNum     = msgON->body.tx_array_car_t.ant ;
    uiCarrier    = msgON->body.tx_array_car_t.car;
    uiCrCentFreq = msgON->body.tx_array_car_t.abs_freq_center;
    uiUnitType   = f_dpdaw_reKind;
    bwhz    = msgON->body.tx_array_car_t.ch_bandwidth;
    gain = msgON->body.tx_array_car_t.gain;

    /********************************************************************/
    /* o-ran-uplane-confのHardcoded-eaxc-id対応により                    */
    /* ACELL-ID, CC-ID, DU-PORT-IDは起動時に設定を行うこととなったが    */
    /* DUコマンド側の変更を抑制するため、non SSB側のみ使用する。        */
    /* なお、f_dpda_car_setTxParameterはコメント化しているため          */
    /* キャリアON時にレジスタ設定は行わない。                           */
    /********************************************************************/
	...
	/* f_dpda_act_txCarrierOnSettingRequest 入力パラメータ変換処理 */
    ...
    uiScs = msgON->body.tx_array_car_t.default_scs;
	switch( uiScs )
	{
		...
		case E_UP_CAR_DEF_SCS_KHZ_240 :						/**< DEF SCS 240kHz			*/
			...
			break ;
		default :											/**< その他					*/
			BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txon_request (SCS)parameter Error. SCS=0x%08x", uiScs);
	...
	{
		/* duコマンド用 */
		BPF_COM_CMD_SET_TXENDP(uiCarrier,uiAntNum,D_RRH_ON,uiAccellId,uiCcId,uiDuPortId,uiAccellId_ex,uiCcId_ex,uiDuPortId_ex);
		BPF_COM_CMD_SET_TXCC(uiCarrier ,uiAntNum ,uiCrCentFreq, uiBw, uiScs );	
	...
	if(ret != D_DPDA_RTN_COMPLETE)
	{
		BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txon_request error=0x%08x", ret);
		/* 通信用共有メモリを取得する	*/
		...
		if( ret != D_SYS_OK )
		{
			BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txon_request buffer error=0x%08x", ret);
		}
		else
		{
			sndMsg->thdif.uiEventNo		= D_SYS_MSGID_TXON_CFM ;		/* TX-ON(M-Plane)応答				*/
			...
			if(ret == D_DPDA_RTN_INPUTERR)
			{
				...
				memcpy( &( sndMsg->msgErr.msgErr.errorDescription ) , t_dpda_txon_err_input , D_DPDA_UPCNF_ERR_MSG_SIZE ) ;
			}
			else
			{
				sndMsg->msgErr.msgErr.errorCode	= D_DPDA_EDIT_ERR ;			/* エラーコード						*/
				memcpy( &( sndMsg->msgErr.msgErr.errorDescription ) , t_dpda_txon_err , D_DPDA_UPCNF_ERR_MSG_SIZE ) ;
			}

			/* TX-ON(M-Plane)応答を送信する */
			ret = BPF_RU_IPCM_MSGQ_SEND(	D_SYS_THDQID_PF_RFA ,		/* 送信先Thread Queue ID			*/
											(void *)sndMsg );			/* 送信Message						*/

			if( ret != D_SYS_OK )
			{
				BPF_COM_LOG_ASSERT( D_RRH_LOG_AST_LV_ERROR, "f_dpda_act_txon_request failed send message. %d", ret);
			}
		}
