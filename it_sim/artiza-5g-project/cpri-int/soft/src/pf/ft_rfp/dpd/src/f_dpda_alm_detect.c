...
VOID f_dpda_alm_detect(VOID)
{
	...
	UINT					almmsk;						/* アラームマスク中					*/
	UINT					dbgmode;					/* デバッグモード中					*/
	

	almmsk = f_dpdaw_info_all->almmsk;
	dbgmode = f_dpdaw_info_all->taskInfo.flg.dbgmode;

	/* ハードレジスタより、アラーム情報を収集 */
	for( almCnt=0; almCnt<D_DPDA_ALM_NUM; almCnt++)
	{
		if( ( f_dpdar_almmsk_check[almCnt].almmsk & almmsk) != 0 )
		{
			/* almmask */
			continue;
		}
		if( ( f_dpdar_almmsk_check[almCnt].dbgmode & dbgmode) != 0 )
		{
			/* dbgmode mask  */
			continue;
		}
