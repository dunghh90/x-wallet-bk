...
/*         OS_STATUS osStatus = OS_MUTEX_TAKE_T(Zl303xx_ReadWriteMutexId, TAKE_TIMEOUT_MS); */    /* Not the default to timeout */
         OS_STATUS osStatus = OS_MUTEX_TAKE(Zl303xx_ReadWriteMutexId);

         if (osStatus != OS_OK)
         {
            ZL303XX_TRACE_ALWAYS("zl303xx_ReadWriteClose: Unable to take Rd/Wr Mutex; status =%d", osStatus, 0,0,0,0,0);
         ...
         if (osStatus == OS_OK)
         {
            ...
            {
               status = ZL303XX_RTOS_SEM_DELETE_FAIL;
               ZL303XX_ERROR_TRAP("Unable to delete Rd/Wr Mutex");
            }
         }
...
zlStatusE zl303xx_ReadWriteLock(void)
{
   ...
   else
   {
      ...
      ZL303XX_TRACE(ZL303XX_MOD_ID_RDWR, 1,
            "zl303xx_ReadWriteLock:", 0,0,0,0,0,0);
                           ...
                           Uint8T *regBuffer, Uint16T regLen)
{
   ...
   ZL303XX_TRACE(ZL303XX_MOD_ID_RDWR, 2,
         "zl303xx_WriteBuf: register %#lx, data %#lx",
                           ...
                           Uint8T *dataBuf)
{
    ...
    {
        /* 72x Page and address decoding from virtual address */
        address = ZL303XX_MEM_ADDR_EXTRACT_72X(regAddr);
    }
#endif
#if defined ZLS30361_INCLUDED
    if(zl303xx_Params->deviceType == ZL3036X_DEVICETYPE)
    {
        ...
        address = ZL303XX_MEM_ADDR_EXTRACT_36X(regAddr);
    }
#endif
#if defined ZLS30701_INCLUDED
    if(zl303xx_Params->deviceType == ZL3070X_DEVICETYPE)
    {
        ...
        address = ZL303XX_MEM_ADDR_EXTRACT_70X(regAddr);
    }
#endif
#if defined ZLS30751_INCLUDED
    if(zl303xx_Params->deviceType == ZL3075X_DEVICETYPE)
    {
        ...
        address = ZL303XX_MEM_ADDR_EXTRACT_75X(regAddr);
    }
#endif
#if defined ZLS30341_INCLUDED
    ...
    {
        address = ZL303XX_MEM_DIRECT_ADDR_EXTRACT(regAddr);
    }
#endif
#if defined ZLS30771_INCLUDED
    if(zl303xx_Params->deviceType == ZL3077X_DEVICETYPE)
    {
        ...
        address = ZL303XX_MEM_ADDR_EXTRACT_77X(regAddr);
    }
#endif
   ...
   if (status == ZL303XX_OK)
   {
      ...
      if (chipSelect && chipSelect->csFuncPtr != NULL)
      {
      ...
      ZL303XX_TRACE(ZL303XX_MOD_ID_RDWR, 1,
            "zl303xx_WriteLow: address %0lX, regLen %0lX",
            address, regLen, 0,0,0,0);
...
/* Original
 *  #if defined ZLS30721_INCLUDED
 *    #warning: I2C multi-byte reads on a shared bus may return incorrect data if interrupted, see datasheet "Read Transations"
 *  #endif
 */
      ...
      ZL303XX_TRACE(ZL303XX_MOD_ID_RDWR, 2,
            "zl303xx_ReadLow: address %0lX, regLen %0lX",
            address, regLen, 0,0,0,0);
                                          ...
                                          Uint32T regAddr)
{
   zlStatusE status = ZL303XX_OK;
   Uint32T pageReg = 0;
   Uint8T newPage;   /* Uint8T ensures the page value is in the buffer[0] position for the zl303xx_WriteLow call */
   zl303xx_BooleanE accessPage = ZL303XX_FALSE;
   static Uint8T currentPage = 0;
   ...
   if (status == ZL303XX_OK)
   {
      ...
      if( zl303xx_Params->deviceType == ZL3036X_DEVICETYPE )
      {
         pageReg = ZL3036X_ADDR_PAGE_REG;
         newPage = ZL303XX_MEM_PAGE_EXTRACT_36X(regAddr);

         if (ZL303XX_MEM_ADDR_EXTRACT_36X(regAddr) != 0x7F)
         {
            accessPage = ZL303XX_TRUE;
         }
      }
#endif
#if defined ZLS30701_INCLUDED
      if( zl303xx_Params->deviceType == ZL3070X_DEVICETYPE )
      {
         pageReg = ZL303XX_MEM_ADDR_MASK_70X;
         newPage = ZL303XX_MEM_PAGE_EXTRACT_70X(regAddr);

         if (ZL303XX_MEM_ADDR_EXTRACT_70X(regAddr) != 0x7F)
         {
            accessPage = ZL303XX_TRUE;
         }
      }
      ...
      if( zl303xx_Params->deviceType == ZL3075X_DEVICETYPE )
      {
         pageReg = ZL303XX_MEM_ADDR_MASK_75X;
         newPage = ZL303XX_MEM_PAGE_EXTRACT_75X(regAddr);

         if (ZL303XX_MEM_ADDR_EXTRACT_75X(regAddr) != 0x7F)
         {
            accessPage = ZL303XX_TRUE;
         }
      }
      ...
      {
         pageReg = ZL303XX_ADDR_PAGE_REG;
         newPage = ZL303XX_MEM_PAGE_EXTRACT(regAddr);
      ...
      if( zl303xx_Params->deviceType == ZL3077X_DEVICETYPE )
      {
         newPage = ZL303XX_MEM_PAGE_EXTRACT_77X(regAddr);

         if (ZL303XX_MEM_ADDR_EXTRACT_77X(regAddr) != 0x7F)
         {
            accessPage = ZL303XX_TRUE;
         }
      }
      ...
      if( accessPage == ZL303XX_TRUE )
      {
         /* accessing the ZL device with a page offset so update the page register if necessary */
         if (currentPage != newPage)
         {
            /* Save the change for future reference - if done after then you could infinite loop */
            currentPage = newPage;

            ZL303XX_TRACE(ZL303XX_MOD_ID_RDWR, 2,
                  "zl303xx_SyncRegisterPage: reg %0lX, set page %0lX @ pageReg=0x%x",
                  regAddr, newPage, pageReg, 0,0,0);

            /* Either change the page here or later in your low level driver */
            status = zl303xx_WriteLow(zl303xx_Params, chipSelect,
                                    pageReg, sizeof(Uint8T),
