...
#if defined MPC8313 || defined MPC8260
...
#if defined ZLS30341_INCLUDED
#include "zl303xx_Dpll34xIsrLow.h"
#include "zl303xx_Dpll34xDpllLow.h"
#include "zl303xx_Dpll34xRefLow.h"
#endif
...
static zl303xx_DebugValToStringS dpllPslToString[] = {
      {ZL303XX_DPLL_PSL_P885US,     "885 ns/s" },     /* DEPRECATED */
      {ZL303XX_DPLL_PSL_P885NS,     "885 ns/s" },
...
zlStatusE devRead(zl303xx_ParamsS *zl303xx_Params, Uint32T page, Uint32T regAddr, Uint32T regSize, Uint32T overlay)
{
   ...
   Uint32T value;
   Uint32T addr;
   zl303xx_BooleanE addrValid;
#if defined ZLS30341_INCLUDED
   Uint32T overlayAddr;
#endif

   (void)page;      /* Warning removal */
   (void)overlay;

   if (regSize > sizeof(value))
   {
       fprintf(stderr, "Enforced Max. %u bytes\n", sizeof(value));
   ...
   addrValid = ZL303XX_FALSE;

#if defined ZLS30361_INCLUDED
   if( zl303xx_Params->deviceType == ZL3036X_DEVICETYPE )
   {
       addr = ZL303XX_MAKE_MEM_ADDR_36X(regAddr, value);
       addrValid = ZL303XX_TRUE;
   }
#endif
#if defined ZLS30341_INCLUDED
   if(( zl303xx_Params->deviceType == ZL3034X_DEVICETYPE )
   ...
   {
       addr = ZL303XX_MAKE_MEM_ADDR_70X(regAddr, value);
       addrValid = ZL303XX_TRUE;
   }
#endif
#if defined ZLS30721_INCLUDED
   ...
   {
       addr = ZL303XX_MAKE_MEM_ADDR_72X(regAddr, value);
       addrValid = ZL303XX_TRUE;
   }
#endif
#if defined ZLS30751_INCLUDED
   ...
   {
       addr = ZL303XX_MAKE_MEM_ADDR_75X(regAddr, value);
       addrValid = ZL303XX_TRUE;
   }
#endif
#if defined ZLS30771_INCLUDED
   ...
   {
       addr = ZL303XX_MAKE_MEM_ADDR_77X(regAddr, value);
       addrValid = ZL303XX_TRUE;
   }
#endif

   if( addrValid == ZL303XX_TRUE )
   {
   ...
   {
       printf("No read address for device\n");
   }
...
zlStatusE devWrite(zl303xx_ParamsS *zl303xx_Params, Uint32T page, Uint32T regAddr, Uint32T regSize, Uint32T data, Uint32T overlay)
{
   ...
   zl303xx_BooleanE addrValid;
#if defined ZLS30341_INCLUDED
   Uint32T overlayAddr = 0;
#endif

   (void)overlay;   /* Warning removal */
   ...
   {
       printf("No write address for device\n");
   }
...
static zlStatusE local_DebugHwRefStatus(zl303xx_ParamsS *zl303xx_Params, zl303xx_RefIdE refId)
{
   ...
   if (status == ZL303XX_OK)
   {
      ...
      printf("   ISR Fail Bits:  SCM:%d CFM:%d GST:%d PST:%d\n", refIsr.scmFail,
                                                                 refIsr.cfmFail,
                                                                 refIsr.gstFail,
                                                                 refIsr.pfmFail);
   }
...
static zlStatusE local_DebugDpllStatus(zl303xx_ParamsS *zl303xx_Params, zl303xx_DpllIdE dpllId)
{
   ...
   {
      printf("<Status>\n");
      printf("   Holdover:       %s\n", ZL303XX_BOOL_TO_STR((zl303xx_BooleanE)dpllStatus.holdover));
      printf("   Locked:         %s\n", ZL303XX_BOOL_TO_STR((zl303xx_BooleanE)dpllStatus.locked));
...
static zlStatusE local_DebugDpllConfig(zl303xx_ParamsS *zl303xx_Params, zl303xx_DpllIdE dpllId)
{
   ...
   if (status == ZL303XX_OK)
   {
      dpllConfig.waitToRestore = 0;  /* coverity warning removal since waitToRestore scalar is uninit */
   ...
   {
      printf("<Configuration>\n");
      printf("   Hitless Ref. Sw.: %d (%s)\n", dpllConfig.hitlessSw,     ZL303XX_BOOL_TO_STR((zl303xx_BooleanE)dpllConfig.hitlessSw));
