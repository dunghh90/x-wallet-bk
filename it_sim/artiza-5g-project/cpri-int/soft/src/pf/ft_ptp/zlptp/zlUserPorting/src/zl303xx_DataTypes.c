...
/**
  ...
  Details:
   Routine to shift 64-bit integers either right or left.

  Parameters:
       val        The value to shift
       shift      +ve shifts left
                     -ve shifts right

  Return Value:
   Uint64S     The shifted number
...
Uint64S zl303xx_SShift_U64S(Uint64S val, Sint32T shift)
{
   ...
   if (shift > 0)
   {
      ret = LShift_U64S(val, shift);
      if ((val.hi & 0x80000000) != (ret.hi & 0x80000000))
      {
         return val;
      }
   }
   ...
   {
      ret = RShift_U64S(val, -shift);
      if (val.hi & 0x80000000)
         ret.hi |= 0xFFFFFFFF << (32 + shift);
   }
   else
   {
      ret = val;
...
/**
  ...
  Details:
   Routine to negate a 64-bit integer.

  Parameters:
       a          The value to negate

  Return Value:
   Uint64S     The negated number

*******************************************************************************/

Uint64S zl303xx_Neg_U64S(Uint64S a)
{
   const Uint64S zero64 = {0,0};

   return Diff_U64S(zero64, a, 0);
}
...
/**
  ...
  Details:
   Routine to return the absolute value of a 64-bit integer.

  Parameters:
       a          The value to take the abolsute value of

  Return Value:
   Uint64S     The absolute value of the number

*******************************************************************************/

Uint64S zl303xx_Abs_U64S(Uint64S a)
{
   return (a.hi & 0x80000000) ? zl303xx_Neg_U64S(a) : a;
}
