...
#define ZL303XX_ALGO_MAX_PARAMETERS 200

#define ZL303XX_ALGO_MAX_MODIFIED_PARAMETERS 10
...
#define ZL303XX_APR_QIF_TASK_STACK_SIZE  (Uint32T)7000
...
typedef Sint32T (*swFuncPtrUserDelay)(Uint32T, Uint64S, Uint64S, Uint64S*);
...
typedef enum
{
    ...
    ZL303XX_APR_LAST_SUPPORT_OSCI_FILTER = ZL303XX_TCXO_FAST_ENHANCED
...
typedef enum
{
    ...
    ZL303XX_BW_23uHz,
    ZL303XX_BW_700uHz,
    ZL303XX_LAST_SUPPORT_FILTER = ZL303XX_BW_700uHz   /* ZL303XX_BW_23uHz */
...
/* Hybrid interface bindings:
   zlStatusE zl303xx_AprUsrFnGetHwFreqOffset(void *clkGen, Sint32T *deltaFreq)
...
#define ZL303XX_DEFAULT_PSL_1HZ_0 (0x19)
...
typedef struct {
   ...
   /*!  Var: zl303xx_PFConfigS::stepTimeMaxTimePerAdjustment
            ...
            Units: ns
            Default: 1000000000 (equal to setTimeStepTimeThreshold) */
   ...
   /*!  Var: zl303xx_PFConfigS::setTimeRoundingZone
            Sometimes, the calculated 1Hz adjustment value can be just smaller
            then the stepTimeAdjustFreqThreshold. This creates the undesireable
            scenario where a (potentially) large phase is applied using frequency
            changes instead of a larger stepTime() followed by a small frequency
            adjustment.
            The system will round-up the 1Hz adjustment by as much as this value
            so that a stepTime() is issued instead of adjustFreq().
            Units: ns
            Default: 10000ns   */
    ...
    */
    ...
    /*!  Var: zl303xx_PFConfigS::hybridCfOffsetDelay
            In hybrid mode at startup, the calculated average can be forced
            to use hybridCfReadFilter1 for this many intervals.
            unit: intervals of 125ms
            default: 240 (8 interval/sec * 30sec) */
...
typedef enum
{
   ...
   ZL303XX_SERVER_CLK_OUTLIER_FLAG,
...
typedef struct {
   ...
   /*!  Var: zl303xx_AprAddDeviceS::legacyTreatment
          If TRUE, then on timer expiration after setTime(), stepTime(), and adjustTime(),
          no action is taken. If FALSE, then algorithm reset may occur and logs may be
          generated.
          default; TRUE
          */
...
/*!  Var: zl303xx_AprAddServerS::uPeriodThreshold
 ...
 /*!  Var: zl303xx_AprAddServerS::HoldoverFreeze
        Turns on freezing DF upon going into holdover rather than using a buffered averaged value */
    Uint32T HoldoverFreeze;
...
/*!  Var: zl303xx_AprAddServerS::L4ThresholdValue
    ...
    */
    ...
    /*!  Var: zl303xx_AprAddServerS::OFMReRouteThr
         Do not modify unless directed by Microsemi personnel.
    */
    zl303xx_BooleanE OFMReRouteThr;

    /*!  Var: zl303xx_AprAddServerS::OFMReRouteDur
         Do not modify unless directed by Microsemi personnel.
    */
    zl303xx_BooleanE OFMReRouteDur;
...
/** zl303xx_AprSetHoldover
   ...
   Manually set APR to enter to or leave from holdover state for the specified device.
...
typedef struct {
   ...
   /*!  Var: zl303xx_AprTimestampS::offsetFromMaster
            A 64-bit offsetFromMaster value. */
...
/** zl303xx_PFGetTIEClearCurrentOffset
   ...
   This routine returns the currnt TIE-clear offset.

  Parameters:
   [in]  zl303xx_Params      pointer to the zl303xx device data
   [in]  p                 pointer to the PSL/FCL data
