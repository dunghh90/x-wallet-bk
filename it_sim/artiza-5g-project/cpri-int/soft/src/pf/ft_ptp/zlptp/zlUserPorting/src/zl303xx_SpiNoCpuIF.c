...
int BPF_HM_DEVC_PTP_WRITE (Uint32T address, Uint32T data);
int BPF_HM_DEVC_PTP_READ (Uint32T address, Uint32T *data_p);

static Uint32T cpu_spidevice_currentPage=~0;
...
cpuStatusE cpuSpiRead(void *params, Uint32T addr, Uint8T *readValue, Uint16T bytes)
{
...
#ifdef OS_LINUX
   if (page != cpu_spidevice_currentPage)
   {
      /* >>> Write value of 'page' to register 0x7F <<< */
	  BPF_HM_DEVC_PTP_WRITE(0x7f, (unsigned int)page);
      cpu_spidevice_currentPage = page;
   }
   ...
   /* #warning MSCC: cpuSpiRead() missing */
	...
	for( i = 0; i < bytes; i++ ) {
		BPF_HM_DEVC_PTP_READ( (address + i), &data);
		*(readValue + i) = (Uint8T)data;
	}
...
cpuStatusE cpuSpiWrite(void *params, Uint32T addr, Uint8T *data, Uint16T bytes)
{
   ...
   /* #warning MSCC: cpuSpiWrite() missing */
	...
	for( i = 0; i < bytes; i++ ) {
		write_data = (unsigned int)*(data + i);
		BPF_HM_DEVC_PTP_WRITE( (address + i), write_data);
	}
