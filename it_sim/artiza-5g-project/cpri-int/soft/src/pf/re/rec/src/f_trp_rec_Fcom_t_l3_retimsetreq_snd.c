...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_retimsetreq_snd.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_retimsetreq_snd(                   /* 終了コード:R */
    USHORT cpr_no                                             /* CPRI番号:I */
) {
    CMT_TSKIF_CPRISND_RETIMSETREQ* bufp;                /* バッファポインタ */
    UINT   msglen = sizeof(CMT_CPRIF_RETIMSETREQ);//CMD_NUM4;                               /* メッセージ長 */
    ...
    printf( "%s %s %d <cpri>%d\n", __FILE__, __FUNCTION__, __LINE__, cpr_no);
#endif

#ifdef FHM_DEBUG_FOR_IT1
	printf( "**FHM**[%d]%s Start(cpr_no = %d) \n",__LINE__,__FUNCTION__,cpr_no);
#endif
    ...
    bufp->cpridat_retimsetreq.signal_kind = CMD_CPRID_RETIMSETREQ + CMD_NUM1;
    //bufp->cpridat_retimsetreq.signal_kind = htons(CMD_CPRID_RETIMSETREQ + CMD_NUM1);
                                                                /* 信号種別 */
