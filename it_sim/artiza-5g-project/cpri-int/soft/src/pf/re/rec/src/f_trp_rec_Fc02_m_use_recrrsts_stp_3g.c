 ...
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_m_use_recrrsts_stp_3g(                           /* なし:R */
...
//    USHORT       sit_ret;
    ...
    /********************************************/
    tc = f_trp_rec_Fc02_t_use_recrrstsreq_get_3g(            /* 残トライ回数:R */
    );
    
    snprintf(errdata, CMD_NUM48, "[cpr:%02d][TRY:%d][3G]RE Carr sts report T.O.", D_REC_C02_3G_CPR_NO(), tc);
    D_REC_IVLINI();
    D_REC_IVLSET_SIZE(bufp, sizeof(T_MSGHEAD));
    D_REC_IVLOUT(D_TCM_INVLOGLV_WARN1, errdata);

	/* 送受信ログの退避	*/
	f_trp_rec_Fcom_t_l3_LogSave( D_REC_LOGKIND_CARRIER, D_REC_C02_3G_CPR_NO() - 1, D_REC_SAVEKIND_TIMEOUT );
	...
	if (tc > CMD_NUM0) {
	    ...
	    /**************************************************/
	    f_trp_rec_Fc02_t_use_recrrstsreq_dec_3g(                     /* なし:R */
	    );
		return;
	}
    /******************************************************/
    ...
    /******************************************************/
    f_trp_rec_Fc02_t_recrrsts_a_ini_3g(D_REC_INK_RTO);
