...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_it_rescmpprc_snd.c
 * @brief  スレッド間(RE起動完了プロシージャ実行指示)送信
...
/*!
 * @brief  スレッド間(RE起動完了プロシージャ実行指示)送信
 * @note   スレッド間(RE起動完了プロシージャ実行指示)を送信する
 *         - バッファ取得
 *         - バッファ設定(データ)
 *         - スレッド間送信
 * @param  -
 ...
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_t_it_rescmpprc_snd_3g(                           /* なし:R */
) {
    CMT_TSKIF_RESCMPPRCIND  buf;
    CMT_TSKIF_RESCMPPRCIND* bufp = &buf;                       /* バッファポインタ */
    UINT   msglen = sizeof(CMT_TSKIF_RESCMPPRCIND) - sizeof(T_MSGHEAD);
                                                            /* メッセージ長 */
    UINT   msgid  = CMD_TSKIF_RESCMPPRCIND_3G;                 /* メッセージID */
