...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_rerstreq_snd.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_rerstreq_snd(                      /* 終了コード:R */
    USHORT cpr_no,                                            /* CPRI番号:I */
    USHORT tgt,                                               /* 対象指定:I */
    USHORT cmn                                            /* RE共通部指定:I */
) {
    CMT_TSKIF_CPRISND_RERSTREQ* bufp;                   /* バッファポインタ */
	UINT   msglen = sizeof(CMT_CPRIF_RERSTREQ);				/* メッセージ長 */
    ...
    bufp->cprisnd_inf.length   = CMD_CPRLN_RERSTREQ;       /* buffer length */
	
    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_rerstreq.signal_kind = CMD_CPRID_RERSTREQ + CMD_NUM1;    /* 信号種別 */
    bufp->cpridat_rerstreq.kind        = tgt;                   /* 対象指定 */
    bufp->cpridat_rerstreq.recomapp    = cmn;               /* RE共通部指定 */
