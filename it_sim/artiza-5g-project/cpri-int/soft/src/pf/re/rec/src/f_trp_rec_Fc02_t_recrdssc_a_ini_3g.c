...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_recrdssc_a_ini_3g.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fc02_t_recrdssc_a_ini_3g(                             /* 変化:R */
    USHORT ink                                              /* 初期化種別:I */
) {
	T_RRH_CARDST_3G		tCardState = {};
// [16B] add start
    USHORT              cnt;
// [16B] add end
...
// [16B] chg start
	f_cmn_com_cardst_3g_set( D_REC_C02_3G_CPR_NO(), &tCardState );
    // REカード機能部情報を初期化(16スロット)
    for ( cnt = 0; cnt < CMD_SLOTINF_MAX; cnt++ ) 
    {
        f_cmn_com_cardstslt_3g_set( D_REC_C02_3G_CPR_NO(), cnt, &tCardState );
