...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_tc_resltinfreq_ini.c
 * @brief  残トライ回数(REスロット情報報告要求)初期化
...
/*!
 * @brief  残トライ回数(REスロット情報報告要求)初期化
 * @note   残トライ回数(REスロット情報報告要求)を初期化する
 *         - 残トライ回数初期化
 * @param  -
 * @return None
 * @date   2008/07/18 FFCS)Shihzh Create.
 ...
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_t_tc_resltinfreq_ini(                         /* なし:R */
) {
    ...
    {
        f_trp_rec_Wc02_stsmng_tbl.sts[cpr_no_idx].tc.resltinfreq 
          = f_trp_com_Wsysprmp_PF[D_SYSPARA_PF_C11_RESLTINF_RTNUM] + CMD_NUM1;
    }
    else
    {
        /* set default value */
        f_trp_rec_Wc02_stsmng_tbl.sts[cpr_no_idx].tc.resltinfreq 
          = D_TCM_SYSDFT_RESLTINF_RTNUM + CMD_NUM1;
    }
