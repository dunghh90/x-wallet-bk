 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fc02_t_recrdssc_i_ini_3g(                             /* 変化:R */
    USHORT ink,                                             /* 初期化種別:I */
    USHORT slt_no                                         /* スロット番号:I */
) {
	T_RRH_CARDST_3G lwCardStaClr;
...
//    f_trp_rec_Fc02_t_autorst_ccl(cardObj);
	...
	f_trp_com_AutoResetMngTbl_3g[cpr_no_idx].ReAutoRstTimFlg = CMD_FLG_OFF;
	
	cm_MemClr(&lwCardStaClr, sizeof(T_RRH_CARDST_3G));

	/* set share memmory 16B KCN chg */
	f_cmn_com_cardstslt_3g_set(D_REC_C02_3G_CPR_NO(), slt_index, &lwCardStaClr);
