...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_ip_recrdsscntc_snd_3g.c
 * @brief  プロセス間(REカード状態変化通知(セクタ部))送信
...
/*!
 * @brief  プロセス間(REカード状態変化通知(セクタ部))送信
 * @note   プロセス間(REカード状態変化通知(セクタ部))を送信する
 *         - バッファ取得
 *         - バッファ設定(データ)
 *         - プロセス間送信
 *         - バッファ解放
 ...
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_t_ip_recrdsscntc_snd_3g(                         /* なし:R */
    USHORT cpr_no,                                           /* CPRI番号:I */
    USHORT slt_no                                         /* スロット番号:I */
) {
//    T_MSG_RECRDSTSCHG_NTC* bufp;                        /* バッファポインタ */
//    UINT   msglen = sizeof(T_MSG_RECRDSTSCHG_NTC) - sizeof(T_MSGHEAD); 
//                                                            /* メッセージ長 */
//    UINT   msgid  = D_MSG_RECRDSTSCHG_NTC;                  /* メッセージID */
...
//    /* バッファ設定(データ) */
//    /************************/
//    f_trp_rec_Fc02_t_recrdssc_i_get_3g(                              /* なし:R */
//        cpr_no,                                               /* CPRI番号:I */
//        slt_no,                                           /* スロット番号:I */
