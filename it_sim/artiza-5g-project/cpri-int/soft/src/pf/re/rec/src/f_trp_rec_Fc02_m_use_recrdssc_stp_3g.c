 ...
 * @date   2015/08/07 fpt)tan Modify
 * @date   2016/10/04 KCN)hfuku 16B機能追加
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_m_use_recrdssc_stp_3g(                           /* なし:R */
...
//    UINT                uit_ret;
	...
	/******************************************/
    tc = f_trp_rec_Fc02_t_use_recrdstsreq_get_3g(            /* 残トライ回数:R */
        slt_no                                               /* スロット番号:I */
    );

    snprintf(errdata, CMD_NUM48, "[cpr:%02d][slt:%02d][TRY:%d][3G]CardSts T.O.", D_REC_C02_3G_CPR_NO(), slt_no, tc);
    D_REC_IVLINI();
    D_REC_IVLSET_SIZE(bufp, sizeof(T_MSGHEAD));
    D_REC_IVLOUT(D_TCM_INVLOGLV_WARN1, errdata);

	/* 送受信ログの退避	*/
	f_trp_rec_Fcom_t_l3_LogSave( D_REC_LOGKIND_CARD, D_REC_C02_3G_CPR_NO() - 1, D_REC_SAVEKIND_TIMEOUT );
	...
	if (tc > CMD_NUM0) {
	    ...
	    /**************************************************/
	    f_trp_rec_Fc02_t_use_recrdstsreq_dec_3g(                     /* なし:R */
	        slt_no                                           /* スロット番号:I */
	    );
		return;
	}
    ...
    /******************************************************/
    f_trp_rec_Fc02_t_recrdssc_i_ini_3g(
        D_REC_INK_RTO, 
        slt_no
    );
