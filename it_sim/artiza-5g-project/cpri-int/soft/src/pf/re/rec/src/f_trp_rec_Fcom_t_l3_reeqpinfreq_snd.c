...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_reeqpinfreq_snd.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_reeqpinfreq_snd(                   /* 終了コード:R */
    USHORT cpr_no                                             /* CPRI番号:I */
) {
    CMT_TSKIF_CPRISND_EQPSTRREQ* bufp;                  /* バッファポインタ */
	UINT   msglen  = sizeof(CMT_CPRIF_EQPSTRREQ);			/* メッセージ長 */
    ...
    if (sit_ret != CMD_OK) {
        ...
        /* 無効処理履歴 */
        snprintf(errdata, CMD_NUM48, "[ERR][cpr:%02d][siz:%u][ret:0x%02x]Alloc buffer NG.", cpr_no, (UINT)sizeof(*bufp), sit_ret);
        D_REC_IVLINI();
    ...
    bufp->cprisnd_inf.length   = CMD_CPRLN_EQPSTRREQ;      /* buffer length */

    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_eqpstrreq.signal_kind = CMD_CPRID_EQPSTRREQ + CMD_SYS_S3G;
                                                                /* 信号種別 */
