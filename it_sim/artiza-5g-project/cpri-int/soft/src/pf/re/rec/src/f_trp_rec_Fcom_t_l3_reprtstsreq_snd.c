...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_reprtstsreq_snd.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_reprtstsreq_snd(                   /* 終了コード:R */
    USHORT cpr_no                                             /* CPRI番号:I */
) {
    CMT_TSKIF_CPRISND_REPORTSTSREQ* bufp;               /* バッファポインタ */
	UINT   msglen = sizeof(CMT_CPRIF_REPORTSTSREQ);			/* メッセージ長 */
    ...
    bufp->cprisnd_inf.length   = CMD_CPRLN_REPORTSTSREQ;   /* buffer length */

    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_reportstsreq.signal_kind = CMD_CPRID_REPORTSTSREQ + CMD_NUM1;
                                                                /* 信号種別 */
