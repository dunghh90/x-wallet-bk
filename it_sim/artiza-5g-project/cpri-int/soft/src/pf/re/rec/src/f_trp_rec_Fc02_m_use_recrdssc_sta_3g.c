 ...
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_m_use_recrdssc_sta_3g(                           /* なし:R */
...
//	CHAR   errdata[CMD_NUM48];
    ...
    /* 16B(0)周期カウンタ加算(+1) */
    timeRecrdcscCount3g[cpr_no_idx] += CMD_NUM1;
    counter = timeRecrdcscCount3g[cpr_no_idx];
    if (timeRecrdcscCount3g[cpr_no_idx] >= CMD_NUM10)
    {   /* カウンタが10になったら、周期カウンタを0にしておく */
        timeRecrdcscCount3g[cpr_no_idx] = CMD_NUM0;
    }
    ...
    /* 16B(2)slot数を抽出 */
    slot_num = f_trp_rec_Wcom_remsc_tbl_3g.remsc[cpr_no_idx].resltinf.slt_num;

#if 0 /* 3Gが自律リセット対応する場合は有効化する */
    if(counter == CMD_NUM1)
    {/* 1回目の時、48時間経過チェックのベース時刻を取得して (次の１回目で再度取得する) */
        BPF_RU_UTTM_GETTIME(&rstBaseTim3g[cpr_no_idx]);
    }
    ...
    {/* 1～8回目の時は、各slotへ送信処理を行う */
        ...
        {
            slot_no1 = f_trp_rec_Wcom_remsc_tbl_3g.remsc[cpr_no_idx].resltinf.slt_no[slot_snd1 - 1];

            /******************************************/
            /* L3(REカード状態報告要求(セクタ部))送信 */
            /******************************************/
            f_trp_rec_Fc02_t_l3_recrdsscreq_snd_3g(                      /* なし:R */
                slot_no1                                         /* スロット番号:I */
            );
            timeRecrdSndCount3g[cpr_no_idx] |= slot_bit[slot_snd1 - 1]; /* REカード状態報告(3G) 送信slotのBitをON */
        }
        /* 送信#2のslot番号を算出 */
        ...
        {
            slot_no2 = f_trp_rec_Wcom_remsc_tbl_3g.remsc[cpr_no_idx].resltinf.slt_no[slot_snd2 - 1];
            ...
            timeRecrdSndCount3g[cpr_no_idx] |= slot_bit[slot_snd2 - 1]; /* REカード状態報告(3G) 送信slotのBitをON */
        }
    }
    else if (counter == CMD_NUM10)
    {
		...
		f_trp_rec_Fc02_t_recrdssc_i_merge_3g(); /* 3GのPort集約(マージ)を行う */


		/* 返信なしのスロットがある場合 */
//		if (timeRecrdSndCount3g[cpr_no_idx] != CMD_NUM0)
//		{
//			snprintf(errdata, CMD_NUM48, "[cpr:%02d][slt:%04x][3G]RE Card sts report No Resp.", D_REC_C02_3G_CPR_NO(), timeRecrdSndCount3g[cpr_no_idx]);
//			D_REC_IVLINI();
