...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_restsreq_snd.c
 * @brief  L3(RE状態報告要求)送信
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_restsreq_snd(                      /* 終了コード:R */
    USHORT cpr_no                                             /* CPRI番号:I */
) {
    CMT_TSKIF_CPRISND_RESTSREQ* bufp;                   /* バッファポインタ */
	UINT   msglen = sizeof(CMT_CPRIF_RESTSREQ);             /* メッセージ長 */
    ...
    bufp->cprisnd_inf.length   = CMD_CPRLN_RESTSREQ;       /* buffer length */

    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_restsreq.signal_kind = CMD_CPRID_RESTSREQ + CMD_SYS_S3G;
                                                                /* 信号種別 */
