...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_tofvalreq_snd.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_tofvalreq_snd(                     /* 終了コード:R */
    USHORT cpr_no                                             /* CPRI番号:I */
) {
    CMT_TSKIF_CPRISND_TOFFSETINFREQ* bufp;              /* バッファポインタ */
	UINT   msglen = sizeof(CMT_CPRIF_TOFFSETINFREQ);//CMD_NUM4;                      /* メッセージ長 */
    ...
    bufp->cprisnd_inf.length   = CMD_CPRLN_TOFFSETINFREQ;  /* buffer length */

    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_toffsetinfreq.signal_kind = CMD_CPRID_TOFFSETINFREQ + CMD_SYS_S3G;
                                                                /* 信号種別 */
