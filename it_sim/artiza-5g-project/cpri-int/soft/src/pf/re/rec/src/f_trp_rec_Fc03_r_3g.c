...
VOID f_trp_rec_Fc03_r_3g(                                            /* なし:R */
   VOID* bufp                                         /* バッファポインタ:I */
    ...
    /* イベント変換 */
    /****************/
    evt = f_trp_rec_Fc03_r_evt_cnv_3g(                           /* イベント:R */
        bufp,                                         /* バッファポインタ:I */
        &evt_cpr_no,                                          /* CPRI番号:O */
        &evt_crr_no                                       /* キャリア番号:O */
    );
    ...
    for (cpr_no = cpr_no_min; cpr_no <= cpr_no_max; cpr_no++) {
        /* パラメータテーブル設定(CPRI番号) */
        f_trp_rec_Wc03_prm_tbl_3g.cpr_no = cpr_no;
        /* 必要キャリア番号数分ループ */
        for (crr_no = crr_no_min; crr_no <= crr_no_max; crr_no++) {
            ...
            /* イベント設定 */
            /****************/
            f_trp_rec_Fc03_t_evt_set_3g(                            /* なし:R */
                evt                                           /*イベント:I */
            );
            ...
            /* 状態番号取得 */
            /****************/
            stn = f_trp_rec_Fc03_t_ownstn_get_3g(                /* 状態番号:R */
            );

            /**********************/
            /* マトリクス関数起動 */
            /**********************/
            f_trp_rec_Rc03_m_3G_fnc[evt][stn](bufp);
        }
    }
