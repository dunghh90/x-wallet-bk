...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_m_use_recrrsts_end_3g.c
 ...
 */
/****************************************************************************/
VOID f_trp_rec_Fc02_m_use_recrrsts_end_3g(                           /* なし:R */
...
//	f_trp_rec_Fcom_t_l3_LogStore( D_REC_LOGKIND_CARRIER, D_REC_C02_3G_CPR_NO(), D_REC_MSGKIND_RSP );
    ...
    if (ret_code != CMD_OK) {
        ...
        if (uit_ret == CMD_ON) {
            snprintf(errdata, CMD_NUM48, "[NOTICE][cpr:%02d][ret:0x%04x]RE Carrier sts report NG.", D_REC_C02_3G_CPR_NO(), ret_code); /* pgr0570 */
            D_REC_IVLINI();
