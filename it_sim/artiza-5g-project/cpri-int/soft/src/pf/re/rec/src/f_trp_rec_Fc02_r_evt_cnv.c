...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_r_evt_cnv.c
 * @brief  イベント変換
 ...
 * @date   2021/01/15 M&C)Huan.dh add case CMD_CPRID_RECARDCNTRES
 * @date   2021/01/21 M&C)Tri.hn Modify CMD_TSKIF_REFRCRSTPRCIND based on No.29 (seq 9-3)
 */
/****************************************************************************/
ET_REC_C02_EVT f_trp_rec_Fc02_r_evt_cnv(                      /* イベント:R */
   ...
   USHORT* cpr_nop                                            /* CPRI番号:O */
    ...
    case CMD_TSKIF_TIMOUTNTC:
        ...
        case EC_REC_TMK_REPRTSTS_INT:
        	evt    = EC_REC_C02_EVT_TO_REPRTSTS_INT;
        	cpr_no = elm.cpr_no;
        	break;
    ...
    printf( "%s %s %d <evt>0x%x\n", __FILE__, __FUNCTION__, __LINE__, cpk);
#endif
        /* Check signal type */
        ...
        case CMD_CPRID_SLOTINFRES:
            ...
            //          cprircv_inf.link_num;
            /* Send msg to L3/log */
	        f_trp_rec_Fcom_t_l3_msg_fw( datp, sizeof(CMT_TSKIF_CPRIRCV_FILSNDFINNTC) );
            break;//
        ...
        case CMD_CPRID_RECARDCNTRES:
			f_trp_rec_Fc02_evt = CMD_ON;
            evt    = EC_REC_C02_EVT_L3_RECRDCNT_RSP;
            cpr_no = ((CMT_TSKIF_CPRIRCV_SIGNALGET*)datp)->
                     cprircv_inf.link_num;
            break;
        ...
        case CMD_CPRID_CARLSTSRES:
            ...
            /*============================================*/
			f_trp_rec_Fc02_evt = CMD_ON;
        	f_trp_rec_Fc02_carr_evt = CMD_ON;
	    ...
	    case CMD_CPRID_MTADDSETRES:
            ...
            if( D_MT_ADD_SET_FACTOR_REC == f_trp_rec_Fc02_t_tc_mtsetffactor_get(cpr_no) ) 
            {
                /*==============*/
                /* イベントなし */
                /*==============*/
    ...
    case CMD_TSKIF_RE_DISCONNECT_REQ:
        if (E_RRH_RAYER_KIND_1_2 == ((CMT_TSKIF_REDIS_REQ*)bufp)->layer_kind) {
            ...
            /* Layer3断(3G)の場合のみリンク断の処理を行う。3Gの断はイベントなし */
            if (CMD_SYS_S3G == ((CMT_TSKIF_REDIS_REQ*)bufp)->system_type) {
    ...
    case CMD_TSKIF_REFRCRSTPRCIND:
        ...
        /* RE強制リセットプロシージャ実行指示 */
        /*====================================*/
/* 2021/01/21 M&C)Tri.hn Modify based on No.29 (seq 9-3) - modify start */
        // evt    = EC_REC_C02_EVT_IT_REFRCRSTPRC_IND;
        // cpr_no = ((CMT_TSKIF_REFRCRSTPRCIND*)bufp)->cpri_no;
        // break;

        /* Get CPRI link status */
        link_sts = ((CMT_TSKIF_CPRSTACHGNTC*)bufp)->link_sts;
        /* Only send RE forced reset if state is StateC */
        switch(link_sts) {
            /* StateC */
...
/* 2021/01/21 M&C)Tri.hn Modify based on No.29 (seq 9-3) - modify end */
    ...
    case CMD_TSKIF_RESYSPARA_UPDATE_RSP:
        if( CMD_SYS_3G == ((CMT_TSKIF_RESYSNEWRES*)bufp)->systype )
        {
            /* 3Gの場合 */
            evt    = EC_REC_C02_EVT_NON;
            cpr_no = D_REC_CPRINO_NON;
    ...
    case CMD_TSKIF_REFIRM_DL_RSP:
        if( CMD_SYS_3G == ((CMT_TSKIF_REFILNEWRES*)bufp)->dummy )
        {
            /* 3Gの場合 */
            evt    = EC_REC_C02_EVT_NON;
            cpr_no = D_REC_CPRINO_NON;
