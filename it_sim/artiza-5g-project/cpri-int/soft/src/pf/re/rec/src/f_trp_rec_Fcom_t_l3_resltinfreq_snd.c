...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_resltinfreq_snd.c
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_resltinfreq_snd(                   /* 終了コード:R */
    USHORT cpr_no                                             /* CPRI番号:I */
) {
    CMT_TSKIF_CPRISND_SLOTINFREQ* bufp;                 /* バッファポインタ */
	UINT   msglen = sizeof(CMT_CPRIF_SLOTINFREQ);           /* メッセージ長 */
    ...
    bufp->cprisnd_inf.length   = CMD_CPRLN_SLOTINFREQ;     /* buffer length */

    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_slotinfreq.signal_kind = CMD_CPRID_SLOTINFREQ  + CMD_SYS_S3G;
                                                                /* 信号種別 */
