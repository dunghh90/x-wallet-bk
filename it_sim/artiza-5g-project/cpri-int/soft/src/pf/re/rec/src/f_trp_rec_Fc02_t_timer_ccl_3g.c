...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_timer_ccl_3g.c
 * @brief  タイマ取消
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fc02_t_timer_ccl_3g(                         /* 終了コード:R */
    ...
    if (tim_id == CMD_NUM0) {
		...
		if(( tmk == EC_REC_TMK_RECRDSSC_CNF ) ||
		   ( tmk == EC_REC_TMK_RECRDSCM_CNF ) ){ /* カード状態報告応答待ちタイマ	*/
			f_trp_rec_Fcom_t_l3_LogSave( D_REC_LOGKIND_CARD, D_REC_C02_3G_CPR_NO() - 1, D_REC_SAVEKIND_TIMESTOPNG );
		}

		else if( tmk == EC_REC_TMK_RECRRSTS_CNF ){	/* キャリア状態報告応答待ちタイマ	*/
			f_trp_rec_Fcom_t_l3_LogSave( D_REC_LOGKIND_CARRIER, D_REC_C02_3G_CPR_NO() - 1, D_REC_SAVEKIND_TIMESTOPNG );
		}
