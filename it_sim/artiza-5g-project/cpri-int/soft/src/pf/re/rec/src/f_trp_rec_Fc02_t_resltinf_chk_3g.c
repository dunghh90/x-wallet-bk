...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_resltinf_chk_3g.c
 ...
 * @retval 1:異常
 * @date   2008/07/21 FFCS)Shihzh create.
 * @date   2015/09/24 FPT)Quynh update for FHM.
 * @date   2016/10/04 KCN)hfuku 16B機能追加
 */
/****************************************************************************/
UINT f_trp_rec_Fc02_t_resltinf_chk_3g(                               /* 変化:R */
...
//    USHORT    slt_cnt           = CMD_NUM0;     /* スロット数ループカウンタ */
    ...
    {
        /* パラメータ範囲外のためアサートを出力 */
        snprintf(errdata, CMD_NUM48, "[NOTICE][cpr:%02d]Param check NG(slotinfnum: %d).", D_REC_C02_3G_CPR_NO(), rcv_slotinfnum);
        D_REC_IVLINI();
    ...
    for(slt_cnt = CMD_NUM0; slt_cnt < rcv_slotinfnum; slt_cnt++)
    {
        ...
        {
            /* パラメータ範囲外のためアサートを出力 */
            snprintf(errdata, CMD_NUM48, "[NOTICE][cpr:%02d]Param check NG(slotno: %d).", D_REC_C02_3G_CPR_NO(), rcv_slotno);
            D_REC_IVLINI();
            D_REC_IVLSET_SIZE(bufp, sizeof(*bufp));
            D_REC_IVLOUT(D_TCM_INVLOGLV_NOTICE, errdata);
            return CMD_NG;
