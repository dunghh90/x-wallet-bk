...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fc02_t_timer_ent.c
 * @brief  タイマ登録
 ...
 * @date   2016/10/04 KCN)hfuku 16B:EC_REC_TMK_RECRDSSC_INTの間隔を200->20に変更
 */
/****************************************************************************/
UINT f_trp_rec_Fc02_t_timer_ent(                            /* 終了コード:R */
    ...
    if (tmk == EC_REC_TMK_REAUTRST_WAT) {
		...
		tim_val = CMD_NUM2 * CMD_NUM10;       /* 16B 200ms->20ms KCN change */
	}
	...
	{
		tim_val = CMD_NUM18 * CMD_NUM10;
	}
	else if (tmk == EC_REC_TMK_REPRTSTS_INT)
	{
    ...
    if (sit_ret != CMD_OK) {
        /* 無効処理履歴 */
        snprintf(errdata, CMD_NUM48, "[ERR][cpr:%02d][slt:0x%02x][err:%d]Timer ent NG.", D_REC_C02_CPR_NO(), slt_no, errcd);
        D_REC_IVLINI();
    ...
    if (uit_ret != CMD_OK) {
        /* 無効処理履歴 */
        snprintf(errdata, CMD_NUM48, "[ERR][cpr:%02d][slt:0x%02x][ret:0x%02x]Timer table ent NG.", D_REC_C02_CPR_NO(), slt_no, uit_ret); /* pgr0570 */
        D_REC_IVLINI();
        D_REC_IVLSET_SIZE(&key_inf, sizeof(key_inf));
        D_REC_IVLOUT(D_TCM_INVLOGLV_ALERT, errdata);
        return CMD_NG;
