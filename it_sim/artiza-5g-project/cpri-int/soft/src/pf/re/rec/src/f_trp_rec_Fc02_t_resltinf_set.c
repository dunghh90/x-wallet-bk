...
UINT f_trp_rec_Fc02_t_resltinf_set(                               /* 変化:R */
    ...
    USHORT  rcv_result;
    USHORT  rcv_slotnum;                           /* スロット数 16B KCN add */
    ...
    /* テーブルREスロット情報アドレス抽出 */
    tblp_sec = &(f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf);
    /* テーブル(BK)REスロット情報アドレス抽出 */
    tbkp = &(f_trp_rec_Wcom_remsc_tbk.remsc[cpr_idx].resltinf);
    /* テーブル→テーブル(BK)にコピー */
    ...
    /* 信号種別設定       */
    /**************************/
    f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.signal_kind = 
        ((CMT_CPRIF_SLOTINFRES*)l3_bufp)->signal_kind;

    /**************************/
    /* 応答結果設定       */
    /**************************/
    f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.result = rcv_result;

    /**************************/
    /* Slot数設定 16B KCN add */
    /**************************/
    f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.slt_num = rcv_slotnum;

    /* 16B KCN chg */
    for (slt_cnt = CMD_NUM0 ; slt_cnt < rcv_slotnum ; slt_cnt++) /* pgr0000 */
    {
        /**************************/
        /* 全スロット番号を保存   */
        /**************************/
        f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.slt_no[slt_cnt] = rcv_slotno[slt_cnt]; /* pgr0000 */
        /******************************/
        /* 全REカード機能部情報を保存 */
        /******************************/
        f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.crdcap[slt_cnt] = rcv_cardfnc[slt_cnt]; /* pgr0000 */
        /***********************************************/
        /* スロット番号に対応したIndex値を格納しておく */
        /***********************************************/
        f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.slt_idx[rcv_slotno[slt_cnt] - 1] = slt_cnt; /* pgr0000 */
    }
	
	/* 共有メモリ(LTE)スロット情報の呼び出し設定用 */
	f_cmn_com_slot_s3g_set( cpr_idx+1,(T_RRH_SLOT_S3G_RE*)&f_trp_rec_Wcom_remsc_tbl.remsc[cpr_idx].resltinf.signal_kind );
