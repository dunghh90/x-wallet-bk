...
/*!
 * @skip   $ld:$
 * @file   f_trp_rec_Fcom_t_l3_txsreq_snd_3g.c
 * @brief  L3(TRX設定要求)送信
 ...
 */
/****************************************************************************/
UINT f_trp_rec_Fcom_t_l3_txsreq_snd_3g(                        /* 終了コード:R */
    USHORT cpr_no,                                            /* CPRI番号:I */
    USHORT crr_no                                        /* キャリア番号:I */
) {
    CMT_TSKIF_CPRISND_TRXSETREQ* bufp;                  /* バッファポインタ */
	UINT   msglen = sizeof(CMT_CPRIF_TRXSETREQ);			/* メッセージ長 */
    ...
    if (sit_ret != CMD_OK) {
        ...
        /* 無効処理履歴 */
        snprintf(errdata, CMD_NUM48, "[ERR][cpr:%02d][crr:%02d][siz:0x%u][ret:0x%02x]Alloc buffer NG.", cpr_no, crr_no, sizeof(*bufp), sit_ret); /* pgr0570 */
        D_REC_IVLINI();
	...
	/* get frq from shared memory since TRX set OK */
    memset(&ltTrxSetPara, 0, sizeof(ltTrxSetPara));
	f_cmn_com_trxset_para_3g_get(crr_no, &ltTrxSetPara);
	frq = ltTrxSetPara.wFreqNo;

    /****************************/
    /* バッファ設定(CPRIヘッダ) */
    /****************************/
    bufp->cprisnd_inf.link_num = cpr_no;                        /* CPRI番号 */
    bufp->cprisnd_inf.length   = CMD_CPRLN_TRXSETREQ;      /* buffer length */

    /****************************/
    /* バッファ設定(CPRIデータ) */
    /****************************/
    bufp->cpridat_trxsetreq.signal_kind = CMD_CPRID_TRXSETREQ;  /* 信号種別 */
    bufp->cpridat_trxsetreq.carno       = crr_no;           /* キャリア番号 */
    bufp->cpridat_trxsetreq.frqno       = frq;                    /* 周波数 */
