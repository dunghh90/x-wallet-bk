/*
 * @file    i_o-ran-uplane-conf_varidate_rx_links.c
 */
...
/*!
 * @brief   rx link array carrierのValidateを行う
 * @note    
 * @param   [in] newval
 * @param   [in] curval
 * @return  [正常]NO_ERR
 ...
 */
status_t i_o_ran_uplane_conf_user_plane_configuration_low_level_rx_links_rx_array_carrier_validate(
    val_value_t *newval,
    const xmlChar *k_user_plane_configuration_low_level_rx_links_name)
{
    ...
    else if (!k_user_plane_configuration_low_level_rx_links_name) {
        if(LOGWARN) {
    ...
    /* array_carrierがsaa_rx_car_tblに未登録の場合はエラーを返す */
    rtn_saa = i_o_ran_uplane_conf_check_array_carrier_from_car_tbl(dir, VAL_STRING(newval));
    if(rtn_saa == D_SYS_NG) {
        if(LOGWARN) {
            log_warn("\n%s: %s is not exist in rx-array-carriers", __func__, VAL_STRING(newval));
        }
        return ERR_NCX_DATA_MISSING;
    }

    /* saa_link_info_tblからlink_name検索 */
    rtn_saa = i_o_ran_uplane_conf_check_link_name_from_link_info_tbl(dir, k_user_plane_configuration_low_level_rx_links_name, &link_info_num);

    /* saa_link_info_tblにlink_name未登録の場合は登録する */
    if(rtn_saa == D_SYS_NG) {
        if (LOGDEV1) {
            log_dev1("\n%s: Add rx links name to control table, name = %s", __func__, k_user_plane_configuration_low_level_rx_links_name);
        }
        rtn_saa = i_o_ran_uplane_conf_set_link_name_to_link_info_tbl(dir, k_user_plane_configuration_low_level_rx_links_name, &link_info_num);
        if(rtn_saa == D_SYS_NG) {
            if(LOGWARN) {
                log_warn("\n%s: Failed to add control table, name = %s", __func__, k_user_plane_configuration_low_level_rx_links_name);
            }
    ...
    if(saa_link_info_tbl[dir][link_info_num].active != E_UP_CAR_ACTIVE_INACTIVE) {
        if(LOGWARN) {
            log_warn("\n%s: Operation Failed, rx links(%s) has active carrier", __func__, k_user_plane_configuration_low_level_rx_links_name);
        }
        return ERR_NCX_RESOURCE_DENIED;
    }

    pair_link_invalid_flag = D_SYS_OK;
    /* saa_link_info_tblにarray_carrierがsaa_upconf_max_prach_num以上登録されている場合はエラーを返す */
    for (uint32 i = 0; i < SAA_UPCONF_MAX_LINK_NUM; i++){
        if (!xml_strcmp((xmlChar *)saa_link_info_tbl[dir][i].array_carrier, VAL_STRING(newval)) && i != link_info_num) {
            ...
            /* Sub 6でBeamforming ONの場合Rx linksに割り付けられるarray-carrierは最大 saa_upconf_max_prach_num*2 */
                if (array_carrier_cnt >= saa_upconf_max_prach_num*2) {
                    if(LOGWARN) {
            ...
            /* Sub 6でBeamforming ON以外の場合Rx linksに割り付けられるarray-carrierは最大 saa_upconf_max_prach_num */
                if (array_carrier_cnt >= saa_upconf_max_prach_num) {
                    if(LOGWARN) {
                        log_warn("\n%s: DATA EXISTS, target array carries(%s) is already used", __func__, VAL_STRING(newval));
                    }
                    return ERR_NCX_DATA_EXISTS;
    ...
    /* saa_rx_link_info_tblにarray_carrier登録 */
...
} /* i_o_ran_uplane_conf_user_plane_configuration_low_level_rx_links_rx_array_carrier_validate */

/*!
 * @brief   rx link endpointsのValidateを行う
 * @note    
 * @param   [in] newval
 * @param   [in] curval
 * @return  [正常]NO_ERR
 ...
 * @date    2019/02/21 KCN)吉木
 */
status_t i_o_ran_uplane_conf_user_plane_configuration_low_level_rx_links_low_level_rx_endpoint_validate(
    val_value_t *newval,
    const xmlChar *k_user_plane_configuration_low_level_rx_links_name)
{
    status_t res = NO_ERR;
    ...
    if(rtn_saa == D_SYS_NG) {
        if(LOGWARN) {
            log_warn("\n%s: %s is not exist in low-level-rx-endpoints", __func__, VAL_STRING(newval));
        }
        return ERR_NCX_DATA_MISSING;
    }
    ...
    if(saa_link_info_tbl[dir][link_info_num].active != E_UP_CAR_ACTIVE_INACTIVE) {
        if(LOGWARN) {
            log_warn("\n%s: rx links(%s) has active carrier", __func__, k_user_plane_configuration_low_level_rx_links_name);
        }
        return ERR_NCX_RESOURCE_DENIED;
    }
    ...
    /* saa_rx_link_info_tblにendpoints登録 */
...
} /* i_o_ran_uplane_conf_user_plane_configuration_low_level_rx_links_low_level_rx_endpoint_validate */