...
/**
 *  @brief  GPIO書き込み
 *  @note   GPIOのWRITE関数を具備する。
 *  @param  loglevel[in]  ログレベル
 *  @param  offset  [in]  GPIO書き込みオフセット
...
int BPF_HM_DEVC_GPIO_WRITE( unsigned int loglevel, unsigned int offset, unsigned int *data_p)
{
    ...
    /* Trace log */
    bpf_com_dbglog_fmt(BPF_COM_LOG_TRACE,"BPF_HM_DEVC_GPIO_WRITE offset = %08x , data_p = %08x" ,
                        offset,*data_p);
