...
/**
 *  @brief  QMOD読み込み
 *  @note   BPF_HM_DEVC_QMOD_READ
 *  @param  ant      [in]   FPGA種別
 *  @param  sel      [in]   セレクタ番号
 *  @param  address  [in]   QMOD読み込みオフセット
...
int BPF_HM_DEVC_QMOD_READ(unsigned char ant, unsigned int sel, unsigned int address, unsigned int *data_p)
{
	...
	switch(sel){
		case BPF_HM_DEVC_TRXSPI_SEL0:
			...
			if(ant == 0){
				ret = bpf_hm_devc_spi_trx_spi_read_mmw(BPF_HM_DEVC_TRXSPI_TARGET_QMOD_V,write_data,data_p);
				devkind = BPF_HM_DEVC_SPI3_SS0;
			}else{
				ret = bpf_hm_devc_spi_trx_spi_read_mmw(BPF_HM_DEVC_TRXSPI_TARGET_QMOD_H,write_data,data_p);
		...
		case BPF_HM_DEVC_TRXSPI_SEL3:
			*data_p = di_devc_data_p->qmod_dac[ant & 3];
			devkind = BPF_HM_DEVC_SPI3_SS6;
			break;
		default:
			bpf_com_dbglog_fmt(BPF_COM_LOG_ERR_L3, "sel Renge NG:%d", sel);
...
/**
 *  @brief  QMOD書き込み
 *  @note   BPF_HM_DEVC_QMOD_WRITE
 *  @param  ant      [in]   FPGA種別
 *  @param  sel      [in]   セレクタ番号
 *  @param  address  [in]   QMOD書き込みオフセット
...
int BPF_HM_DEVC_QMOD_WRITE(unsigned char ant, unsigned int sel, unsigned int address, unsigned int data)
{
	...
	switch(sel){
		case BPF_HM_DEVC_TRXSPI_SEL0:
			...
			if(ant == 0){
				ret = bpf_hm_devc_spi_trx_spi_write_mmw(BPF_HM_DEVC_TRXSPI_TARGET_QMOD_V,write_data);
				devkind = BPF_HM_DEVC_SPI3_SS0;
			}else{
				ret = bpf_hm_devc_spi_trx_spi_write_mmw(BPF_HM_DEVC_TRXSPI_TARGET_QMOD_H,write_data);
				devkind = BPF_HM_DEVC_SPI3_SS3;
			}
			break;
		case BPF_HM_DEVC_TRXSPI_SEL1:
			bpf_com_dbglog_fmt(BPF_COM_LOG_ERR_L3, "Skip setting %d 0x%08x 0x%08x",ant,address,data);
			devkind = BPF_HM_DEVC_SPI3_SS1;
			break;
		case BPF_HM_DEVC_TRXSPI_SEL3:
			write_data = 0;
			...
			di_devc_data_p->qmod_dac[ant & 3] = data;
			devkind = BPF_HM_DEVC_SPI3_SS6;
			break;
		default:
			bpf_com_dbglog_fmt(BPF_COM_LOG_ERR_L3, "sel Renge NG:%d", sel);
