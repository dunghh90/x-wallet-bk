...
/**
 *  @brief  TXDAC読み込み
 *  @note   BPF_HM_DEVC_TXDAC_READ
 *  @param  address  [in]   TXDAC読み込みオフセット
...
{
    /* Trace log */
    bpf_com_dbglog_fmt(BPF_COM_LOG_TRACE,"BPF_HM_DEVC_TXDAC_READ address = %04x" ,address);
    ...
    /* ターゲット設定 */
    target = BPF_HM_DEVC_SPI_TYPE_TXDAC;
...
/**
 *  @brief  TXDAC書き込み
 *  @note   BPF_HM_DEVC_TXDAC_WRITE
 *  @param  address  [in]   TXDAC書き込みオフセット
